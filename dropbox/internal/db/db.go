// Package db holds dropbox's embedded migration set and a thin migrate helper for
...
// §B / E3) — this package keeps only what is app-side: the *.sql files (embedded
// for Spec.Migrations) and the byte-equality guard that 003_outbox.sql matches
// the library DDL (migrations_outbox_test.go).
package db
...
// FS exposes the embedded migration set so cmd/dropbox can hand it to
// appkit.Spec.Migrations (the binary is the source of truth for its own schema).
var FS = migrationsFS

// Open opens dropbox's SQLite database with the chassis pragmas (delegates to
// appkit/db so there is one Open implementation across every service).
func Open(dbPath string) (*sql.DB, error) {
	return db.Open(dbPath)
}

// Migrate applies dropbox's embedded migrations against conn using appkit's
