...
// the /srv/dropbox/ prefix, so the bare path is /.well-known/oauth-protected-resource.
// `resource` is byte-equal to the configured canonical resource id.
func (a *app) handlePRMetadata() http.HandlerFunc {
...
// handleWhoami is the dropbox_whoami proof the dashboard's connect skill uses to
// verify the full auth chain end to end. It is behind requireIdentityHeaders,
// so the identity is always present on the context. No side effects.
func (a *app) handleWhoami() http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
