...
func NewHandler(svc *dropbox.Service, sourcePortAllowed func(int) bool, source *http.Client, rt *appkit.Router) (http.Handler, error) {
	if svc == nil {
	...
	return appkitmcp.New(appkitmcp.Options{
		...
		Tools:         Tools(svc, sourcePortAllowed, source),
		Health:        rt.Health(),
		Events:        rt.Events(),
