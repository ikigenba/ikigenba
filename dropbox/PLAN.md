...
- **No `bin/backup` / `bin/restore`.** Unlike the dashboard (and ralph), dropbox
...
- **nginx fragment** (`etc/nginx.conf`): the standard PRM-open + `auth_request`-
  ...
  (`for svc in crm ledger notify ralph`); **`dropbox` must be added to that loop**
...
5. **Phase 4 — Sync engine + Service + Events (the integration heart).**
   ...
   - *Context:* §2/§5/§6 (the load-bearing correctness rules);
     ...
     `ralph/internal/runner` for the background-goroutine lifecycle (start in
...
7. **Phase 6 — Deploy & config wiring.**
   ...
   - *Context:* §9; `notify/bin/secrets`, `ralph/bin/secrets`; `ledger/bin/{build,
