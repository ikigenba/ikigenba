...
3. **Check coverage.**
   - **Code phase:** for **every** id listed under **Ids to cover**, confirm a
     ...
     `dropbox/etc/nginx.conf` and distinguishes the exact-match `= /srv/dropbox/`
     from the prefix `/srv/dropbox/`). **When uncertain that a test really
   ...
   - **Structural / docs phase** (Ids to cover = "(none — structural phase)"):
     ...
     `grep -i "no UI" dropbox/CLAUDE.md` finds nothing and that `dropbox/CLAUDE.md`
     states the landing-page truth. The green suite plus the named check is the
     bar.
