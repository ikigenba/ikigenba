...
Run from the dropbox service root — your working directory. Design states these as
`cd dropbox && …` because design is read from the repo root; the loop already runs
...
dropbox has a **live layer**: ids whose tests live in files carrying the
`//go:build live` constraint (today `internal/dropbox/client_live_test.go`, carrying R-KEIO-B98F, R-KFQK-P0Z4, R-KGYH-2SPT (the real Dropbox write contract)). Live tests
...
2. **(b) The tree's design Conventions document the invocation and its
   ...
   grep -n 'DROPBOX_APP_KEY' project/design/README.md          # must print at least one line
   grep -n 'DROPBOX_APP_SECRET' project/design/README.md       # must print at least one line
   grep -n 'DROPBOX_REFRESH_TOKEN' project/design/README.md    # must print at least one line
   ```

   The documented credentials are `DROPBOX_APP_KEY`, `DROPBOX_APP_SECRET` and `DROPBOX_REFRESH_TOKEN`.
...
- **Never** write outside the `dropbox/` tree, and never run the suite's shared
...
- `message` — one short, plain sentence describing what happened, e.g. `Phase 38
