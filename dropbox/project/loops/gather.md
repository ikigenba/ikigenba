...
- **Test placement — co-locate, never phase-name.** A phase is one package, so
  its tests live in that package: `dropbox/internal/web/web_test.go`,
  `package web`, each named for the behavior it asserts — never in a root-level or
  `phaseNN_test.go` file. A config-artifact test (the nginx fragment) reads
  `dropbox/etc/nginx.conf` from disk and asserts over its content; place it in the
  `web` package (e.g. `dropbox/internal/web/nginx_test.go`) or another focused
