# sites/project — workspace layout

Everything the sites service needs to be **designed, planned, and built**
...
root** (`sites/`), which is also the directory the `ralph` build loop runs
from.
...
| `plan/` | `README.md` (spine) + `STATUS.md` (the manifest — the `Next phase` counter plus the only home of each pending phase's `⬜` marker) + `phase-NN.md` (one per **pending** phase) | `$seal-spec` (appends; the build loop deletes a phase's line + body file on completion) |
...
- **gather** — the only prompt that reads the big docs. Greps `STATUS.md` for
  the first `⬜` phase; if there is none the queue is empty and it returns `DONE`
  (the sole exit). Otherwise it resolves that phase's Decision(s) and writes a
  tiny, self-contained `loops/brief.md`, then returns `NEXT`.
...
- **verify** — the independent gate and only prompt that mutates `STATUS.md`.
  ...
  file, commit the deletion; gap → leave the `⬜` line as is. Either way it
  deletes `loops/brief.md`. Returns `NEXT`.
