...
// routing tools/call to the injected script.Service — the handler talks only to
// Service, never to the store directly, so ownership is enforced in one place.
type Handler struct {
	svc     *script.Service
	version string
	service string
...
// NewHandler builds a Handler over the script domain service. version/service/
// health populate the shared health envelope: version and service fill its
// required top-level keys, and the optional health reporter populates its
// details object (the static runtime contract).
func NewHandler(svc *script.Service, version, service string,
...
func (h *Handler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	case "initialize":
		writeJSONRPCResult(w, req.ID, map[string]any{
			...
			"serverInfo":      map[string]any{"name": "Scripts", "version": "1"},
