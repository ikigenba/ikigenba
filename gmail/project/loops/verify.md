...
3. **Check coverage.**
   - **Code phase:** for **every** id listed under **Ids to cover**, confirm a
     ...
     `gmail/etc/nginx.conf` and distinguishes the exact-match `= /srv/gmail/` from
     the prefix `/srv/gmail/`). **When uncertain that a test really asserts, treat
     the id as uncovered.**
   - **Structural / docs phase** (Ids to cover = "(none — structural phase)"):
     run the named content check instead — e.g. confirm
     `grep -i "no UI" gmail/AGENTS.md` finds nothing, that `gmail/AGENTS.md` states
     the landing-page truth, and that `gmail/CLAUDE.md` still resolves to
     `gmail/AGENTS.md` (the symlink is intact). The green suite plus the named
     check is the bar.
