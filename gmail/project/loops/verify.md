...
feedback in front of `build`. The loop's only exit is gather finding no `⬜`
phase.
...
4. **Decide:**
   ...
   - **Gap** (any check failed or any id not convincingly covered): leave the `⬜`
     ...
     `N`, its recorded build commit, and its prior open-gap id set. Capture the
     current build commit (`git rev-parse HEAD`). *No progress* this cycle means
