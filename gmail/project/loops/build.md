...
3. **Do one increment of the remaining work.** Build the package(s) / artifact
   ...
   **code** phase, write id-tagged, genuinely-asserting tests: each Verification
   ...
   `gmail/internal/web/web_test.go`, `package web`, named for the behavior — never
   in a root-level or `phaseNN_test.go` file. The nginx-fragment test reads
   `gmail/etc/nginx.conf` from disk and asserts over its content; keep it in a
   focused package (e.g. `gmail/internal/web`). For a **docs/structural** phase,
   ...
   - **Composition root.** `cmd/gmail/main.go` is grown incrementally (e.g. adding
     the `GET /{$}` landing route to the existing `Handlers` hook) — that is
     wiring growth, not a domain rewrite. Leave the Gmail client + producer Engine
     construction, the `POST /mcp` mount, and the `Producer`/`Workers` hooks
     (the outbox sink and the poll daemon) intact.
   - **AGENTS.md / CLAUDE.md.** They are one file (`gmail/CLAUDE.md` is a symlink to
     `gmail/AGENTS.md`). Edit **`AGENTS.md`**; a refusal to write through the
     symlink is expected.
...
- **Toolchain:** Go 1.26, single `module gmail` rooted at `gmail/`; pure-Go SQLite
