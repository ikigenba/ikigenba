// Package db holds gmail's embedded migration set and a thin migrate helper for
...
// Spec.Migrations) and the byte-equality guard that 003_outbox.sql matches the
// library DDL (migrations_outbox_test.go).
package db
...
// Open opens gmail's SQLite database with the chassis pragmas (delegates to
// appkit/db so there is one Open implementation across every service).
func Open(dbPath string) (*sql.DB, error) {
	return db.Open(dbPath)
}

// Migrate applies gmail's embedded migrations against conn using appkit's
