...
func TestRunRequeuesOrphanedWorkingJobOnStartup(t *testing.T) {
	...
	jobID, err := svc.Ingest(ctx, "default", "owner-id", "owner@example.com", "Acme Robotics reopened a stuck worker job.", "Stuck job", nil)
	if err != nil {
		t.Fatalf("Ingest: %v", err)
	}
...
func TestRunAbortWorkingJobCancelsProviderAndPreservesAbortedStatus(t *testing.T) {
	...
	jobID, err := svc.Ingest(ctx, "default", "owner-id", "owner@example.com", "Acme Robotics opened a blocked lab.", "Blocked lab", nil)
	if err != nil {
		t.Fatalf("Ingest: %v", err)
	}
