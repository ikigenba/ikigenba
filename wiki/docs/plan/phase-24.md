...
- `internal/wiki` — `LLMCallStore`, a SQLite-backed `Recorder` over a new `llm_calls` table (new migration via `bin/new-migration wiki`, matching the live TEXT-`RFC3339Nano` / no-`FOREIGN KEY` style, with the `llm_calls_job` and `llm_calls_time` indexes). Records are append-only — no update, no delete.
