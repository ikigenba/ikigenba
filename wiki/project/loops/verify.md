...
Run from the wiki service root — your working directory. Design states these as
`cd wiki && …` because design is read from the repo root; the loop already runs
...
wiki has a **live layer**: ids whose tests live in files carrying the
`//go:build live` constraint (today `internal/llm/embed_live_test.go` and `autotune/folders_live_test.go`, carrying R-15NY-IF46 (the prompts `/embed` smoke), R-AFK7-HJ0T and R-AGS3-VARI (the real judge-model smokes)). Live tests
...
2. **(b) The tree's design Conventions document the invocation and its
   ...
   grep -n 'WIKI_LIVE_PROMPTS_URL' project/design/README.md    # must print at least one line
   grep -n 'OPENAI_API_KEY' project/design/README.md           # must print at least one line
   ```

   The documented credentials are `WIKI_LIVE_PROMPTS_URL` and `OPENAI_API_KEY`.
...
- `message` — one short, plain sentence describing what happened, e.g. `Phase 125
