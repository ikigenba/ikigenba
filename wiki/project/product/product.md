...
The wiki is ikigenba's knowledge base: an MCP service that *compiles* ingested text into durable per-subject pages and answers questions from them. Its primary surface is **MCP** (agents work through tools, not screens), but it also serves a small **human web surface** at its mount root: a logged-in person who opens `…/srv/wiki/` in a browser gets a search box to **ask** the wiki and read a cited answer, and can **follow links** from there into per-subject pages and onward through the subject graph. The web is **read-only navigation** — ask and browse, nothing that changes content — a styled human window onto the same `ask` and pages the MCP surface already exposes; all mutating and operational work stays on MCP. Each ingest breaks text into the **subjects** it concerns and the **claims** it makes, then writes or updates each subject's page, so knowledge **compounds** — the LLM does the upkeep humans abandon; the user curates what goes in and asks. The owner can **merge** two names for the same thing into one page. Around that core the wiki records **every LLM call** and lets the user steer the ingest queue (list, abort, re-run); reads are always served and never wait on the slow, strictly-serial ingest work. Every call site is configured **independently**, and the developer can take a call site **off-line and evaluate** it against a human-blessed gold dataset to choose what runs in production on evidence rather than guesswork.
...
- **Ask on the web is the same ask as on MCP, and reads like an article.** Type a question and the page becomes the cited answer drawn only from compiled pages — or a plain "nothing here" when the wiki holds nothing — with an "ask another question" control above it that returns you to the empty home; the subjects the answer names are gathered in a footer you can follow into their pages.
- **Every page is a node you can navigate from, and navigation changes nothing.** A subject page shows its compiled prose with a footer of the subjects it points to (named in its prose) and the subjects that point to it; following any of them lands on that subject's page, with the same "ask another question" escape back to home. The web only reads — it never alters a subject, claim, page, or job.
