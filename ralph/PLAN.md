# ralph — build plan (run-once slice)
...
1. **Deliverable of the build:** a **verified local end-to-end run** (the fusion
   example over dev nginx :8080 → ralph :3004, real Anthropic key). Box-deploy
   scripts are *written and reviewed* but a real box deploy is a **manual
   post-step**, out of scope for the orchestrated build.
2. **Adapt, don't freeze-copy.** ikigai-cli's engine is *fitted* to ralph, not
   mirrored. Editing the borrowed code is expected and fine — ralph owns its copy.
3. **Agent loop:** adapt `agent.Run` **in place** — make the schema optional, add
   ...
   `FramingPrompt` for a ralph framing prompt, and thread `sandboxRoot` + `ctx`
...
6. **Service location:** the service lives in this `ralph/` directory (Go module
   `ralph`), beside `README.md` / `ARCHITECTURE.md` / `.envrc`. Added to the root
   `go.work`.
...
- **`nginx/`** dev front door (`run` includes `locations/*.conf`); clone
  `locations/ledger.conf` → `locations/ralph.conf`.
...
- Read `ikigai-cli` engine packages; produce the **copy manifest**: every file to
  copy into `ralph/internal/engine/`, the import-rewrite map
  (`github.com/ai4mgreenly/ikigai-cli/internal/… → ralph/internal/engine/…`), and
...
**Output artifact:** `ralph/.plan/manifest.md` (copy list, import map, closure,
...
**Goal:** ralph stands up as a renamed ledger: builds, boots, `ralph_whoami` works.

**Subagent work (per the Phase 0 rename map):** copy `ledger/`'s code into
`ralph/` (merging with the existing docs/`.envrc`); rename module `ledger`→`ralph`,
`cmd/ledger`→`cmd/ralph`, `LEDGER_`→`RALPH_`, port `3002`→`3004`,
`/srv/ledger/`→`/srv/ralph/`, tool prefix `ledger_`→`ralph_`, `ledger.db`→`ralph.db`,
`/opt/ledger`→`/opt/ralph`, systemd unit `ledger`→`ralph`. Update
`etc/{manifest.env,deploy.env,nginx.conf}`, `Makefile`, `bin/*`. Add `ralph` to the
root `go.work`. Keep `db_test.go`/`server_test.go` green.

**Gate:** `cd ralph && go build ./... && go test ./...` green; binary boots on
`:3004`; `GET /whoami` + `tools/list` over `/mcp` returns `ralph_whoami`.

## Phase 2 — Engine copy-in (verbatim, compiling)

**Goal:** the borrowed engine lives under `ralph/internal/engine/` and compiles
...
**Gate:** `go build ./...` green (engine compiles under `ralph`); any copied
engine tests green.

## Phase 3 — Engine adaptation (freeform loop + framing + confinement seam)

**Goal:** the engine behaves the way ralph needs.
...
- `prompt.go`: replace `FramingPrompt` with a **ralph framing prompt** — agent
...
**Subagent work:** add `internal/db/migrations/002_ralph.sql` exactly per
`ARCHITECTURE.md` §4 (`sessions`, `runs`, `idx_runs_session`). Create the `data/`
layout convention (`ralph.db`, `sandboxes/<id>/`, `runs/<id>/<run>.jsonl`).
...
goroutine: `context.WithTimeout(parent, RALPH_RUN_TTL)`; open
`data/runs/<id>/<run>.jsonl` as a `wire.Session` sink; build the Anthropic client
(`os.Getenv("ANTHROPIC_API_KEY")`) + `provider.Request` (ralph framing + prompt +
...
**Goal:** the full `ralph_*` tool surface wired to the service.
...
`dispatchTool` routes each `ralph_*` name to a `Service` method (and
`session_output` → `runs.log_path` line-slice; `fs_list`/`fs_read` →
`sandbox`), marshaling results to MCP content. `ralph_whoami` retained.
...
**Goal:** `cmd/ralph/main.go` assembles everything and boots.
...
`RALPH_*` config incl. `RALPH_RUN_TTL`.

**Gate:** ralph boots on `:3004`; `tools/list` returns all 11; `session_create`
...
provisioning; `secrets` does the non-destructive RMW of ralph's
...
**Gate:** `shellcheck` clean (or reviewed); `bin/start`/`bin/stop` drive ralph
...
**Subagent work (driven by the orchestrator):** add `nginx/locations/ralph.conf`
(clone of `ledger.conf`); start dashboard (:3000) + ralph (:3004) + nginx (:8080);
register ralph in the dashboard's `DASHBOARD_RESOURCES` for local dev; run the
flow: `ralph_session_create {prompt, config}` → `ralph_session_run` → poll
`ralph_session_get.last_run` until terminal → `ralph_session_output` (the narrated
jsonl) → `ralph_session_fs_list` / `ralph_session_fs_read` the deliverable the
agent wrote. `.envrc` supplies `ANTHROPIC_API_KEY` (run `direnv allow ralph/`).
