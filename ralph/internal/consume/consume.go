// Package consume is ralph's event-plane CONSUMER domain: it turns cron's
// time events into ralph runs. It is the mirror of notify's internal/push —
// where notify reacts to crm's contact.created with a best-effort push, ralph
...
// worker, no crash-recovery sweep beyond ralph's existing orphaned-run sweep).
...
// cron decisions §2) ralph needs: scheduled_for drives the staleness guard.
...
// Subscription is ralph's declared event-plane in-edge: it listens to every
...
func Subscription() consumer.Subscription {
	return consumer.Subscription{
		Source:      "cron",
		Filter:      "cron.*",
		Description: "starts a run for every ralph session whose trigger matches the fired cron event (in-memory fire-and-run)",
	}
}

// Handler returns the consumer.Handler ralph hands to the engine. fire starts a
