// Package db holds ralph's embedded migration set and a thin Open/Migrate helper
...
// for Spec.Migrations) and the ralph_schema_test that asserts 002_ralph.sql lands
...
// FS exposes the embedded migration set so cmd/ralph can hand it to
// appkit.Spec.Migrations (the binary is the source of truth for its own schema).
var FS = migrationsFS

// Open opens ralph's SQLite database with the chassis pragmas (delegates to
// appkit/db so there is one Open implementation across every service).
func Open(dbPath string) (*sql.DB, error) {
	return db.Open(dbPath)
}

// Migrate applies ralph's embedded migrations against conn using appkit's
