...
// table DDL is OWNED by the eventplane library (outbox.SchemaSQL); ralph's
// 005_outbox.sql migration only applies it. If the two drift, ralph's outbox is
// no longer identical to every other producer's — so this test fails loudly the
// moment they diverge (mirrors crm/ledger/cron). ralph is the event-plane
