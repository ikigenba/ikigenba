...
// Outcome event types (event-triggering decisions §3). ralph emits exactly two
// terminal, STATIC, compile-time-known outcome types (source = ralph) — two
// distinct types, NOT one type with a status field, so a consumer can filter at
// the type level. They match ralph's runs.status vocabulary
...
// Events is ralph's published-event Registry, wired via Spec.Events (STATIC —
...
var Events = outbox.Registry{
	{
		...
		Description: "A ralph run finished successfully. Carries the session identity, the human-readable task name, and the trigger context (the cron event + scheduled slot that started it, empty for a manual run).",
		Sample:      sampleOutcomeSuccess,
	},
	{
		Type:        EventRunFailed,
		Description: "A ralph run terminated in failure. Same shape as run.succeeded plus an error string describing the terminal failure.",
...
// ralph (read via MCP); this is the minimal-for-now outcome shape.
