...
discover the real root (`/opt/ralph/data/data/sandboxes/<session>/`). That wasted
...
- **A real download channel**: a short-lived, authenticated HTTP GET for a
  sandbox file (`GET /srv/ralph/sessions/{id}/fs/{path}`) that streams bytes
...
- Decide the **retention / cleanup** policy for sandbox files and run logs so the
  box doesn't fill up (`/opt/ralph/data/...`).
...
- **Reuse the event plane.** The suite already has an outbox/SSE event-plane
  (`eventplane/`, crm is a producer, notify is a consumer + push). Ralph emitting
...
- **Observability for the operator.** Trace logging exists (`engine/trace`);
  make sure run failures land somewhere greppable (`journalctl -u ralph`) with
