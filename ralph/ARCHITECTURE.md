# ralph — architecture (draft 1, run-once slice)
...
ralph is assembled from three sources, matching the README's "reuses vs. new":

| Layer | Source | Disposition |
|---|---|---|
| **Chassis** — loopback HTTP, nginx identity gate, PRM doc, MCP JSON-RPC, SQLite + migrations, `bin/*` lifecycle | `../ledger` | clone + rename `ledger`→`ralph` |
...
upstream** once borrowed — so ralph owns its copy outright (no sync-back).
...
```
ralph/
├── cmd/ralph/main.go              # composition root  (← ledger, renamed)
├── go.mod   (module ralph)        # engine is stdlib-only, so no new deps
├── Makefile · bin/* · etc/*       # PORT=3004, MOUNT=/srv/ralph/, RALPH_* env
└── internal/
...
- **`cmd/ralph/main.go`** — composition root. Reads `RALPH_*` env, opens SQLite
  (`db.Open` — WAL, FK, single writer), runs embedded migrations
  (`db.Migrate`), builds the MCP handler, builds the HTTP server
  (`server.New`), serves with graceful shutdown. ralph adds two wiring steps
...
1. `ctx, cancel := context.WithTimeout(parent, RALPH_RUN_TTL)` — the TTL is the
...
1. Copy these packages from `~/projects/ikigai-cli/app-root/internal/` into
   `ralph/internal/engine/`: `provider/` (incl. `provider.go` and
...
2. Rewrite import paths: `github.com/ai4mgreenly/ikigai-cli/internal/… →
   ralph/internal/engine/…` (mechanical sed across the copied tree).
...
ralph ships **setup / deploy / start / stop / secrets / backup / restore /
teardown** (ledger had only build/deploy/setup/start/stop). ralph is the first
...
- **`bin/setup`** — adds idempotent Python provisioning after the app-user /
  `/opt/ralph` tree and before the nginx fragment:
  ```sh
...
- **`bin/backup` / `bin/restore`** — new. ralph holds durable state the run-once
  slice can lose on box replacement: the SQLite DB (`data/ralph.db` — sessions +
  ...
  the snapshot alongside the folders, rather than copying the hot `ralph.db`.
  Sandbox/run folders sync as-is (forward-only files).
- **`bin/teardown`** — new. Reverse of setup: stop+disable the unit, remove
  `/opt/ralph` (incl. `data/` — DB **and** sandbox folders), drop the nginx
  fragment, `nginx -t` + reload. Does **not** uninstall python3 (shared box
  resource ralph didn't exclusively own).
- **`bin/start` / `bin/stop`** (local dev) — add a soft `python3` preflight
  warning; non-fatal (ralph boots fine without it; only the agent toolset needs it).
...
- **`websearch` / `webfetch`** — the ralph-mediated network tools are **not** in
