...
happened — a restart of a disabled app reads `service: ok (crm v0.1.0
disabled)`, and a disable of an app already disabled says `already
inactive` — which a fixed line of devctl's would hide. A failure is the
