...
- R-PX9J-BIQU: `space create` MUST write the step `address` sixth, after calling `session.Clients.EC2.AllocateAddress(ctx, root.Domain, sp.Domain)` exactly once and then `space.WaitAssociate(ctx, deps, session.Clients.EC2, allocationID, instanceID)` exactly once with the returned `cloud.Address`'s `AllocationID` and the running instance's `ID`, and its detail MUST be `elastic ip `, that address's `IP`, and ` associated`; when either call returns a non-nil error create MUST return it unchanged, write no `address` line, and call `ReleaseAddress`, `TerminateInstance`, and `DeleteRole` not at all; verified at least by reproducing `address: ok (elastic ip 18.118.7.42 associated)`, including for a fake `EC2` whose `AssociateAddress` fails with a `*cloud.Error` whose `Code` is `InvalidAllocationID.NotFound` on its first call and then succeeds, and by a fake `EC2` whose `AllocateAddress` fails with a `*cloud.Error` for `ec2` `AllocateAddress` with the code `AddressLimitExceeded` giving the single stderr line `devctl: ec2 AllocateAddress: AddressLimitExceeded`, exit 1, and no later call.
