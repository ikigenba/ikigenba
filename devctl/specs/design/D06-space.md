...
- R-VJUO-DMNT: When `LookupSpace` found an instance and `--no-backup` is not given, the `retire` step of `space destroy` MUST make exactly one call to `(host.Host).Sudo` with the step `retire` and exactly the arguments `opsctl` and `retire`, on a `host.Host` whose `Address` is the space's `Address` and whose `Deps` is `deps`, before any method of `clients` that terminates, releases, deletes, or changes anything other than the `apex` step's record delete; when that call returns a non-nil error destroy MUST return it unchanged, write no `retire` line and no later line, and make none of those later calls, so that the instance, the address, the space's records, its secrets, its backups, and its role are as they were and the diagnostic quotes the host's report; verified at least by reproducing the stderr first line `devctl: retire: ssh ec2-user@18.220.10.5 sudo opsctl retire: exit status 1`, followed by an empty line and the fake process's output with each line prefixed `> `, with empty stdout and exit 1 for a space that does not hold the apex.
