...
- R-8M1G-J2EM: When `LookupSpace` found an instance and `--no-backup` is not given, the `retire` step of `space destroy` MUST make exactly one call to `(host.Host).Sudo` with the step `retire` and exactly the arguments `opsctl` and `retire`, on a `host.Host` whose `Address` is the space's `Address` and whose `Deps` is `deps`, before any method of `clients` that terminates, releases, deletes, or changes anything other than the `apex` step's record delete; when that call returns a non-nil error destroy MUST return that `*host.CommandError`, with its step `retire`, its status, and the host's output as the call produced them, write no `retire` line and no later line, and make none of those later calls, so that the instance, the address, the space's records, its secrets, its backups, and its role are as they were and the diagnostic quotes the host's report; verified at least, for a space at `18.220.10.5` that does not hold the apex, by a fake `sudo opsctl retire` process that exits 1 writing to its standard output exactly the two lines `a` and `b` and to its standard error exactly the line `c`, each followed by a newline, leaving stdout empty and writing to stderr exactly the five lines `devctl: retire: ssh ec2-user@18.220.10.5 sudo opsctl retire: exit status 1`, an empty line, `> a`, `> b`, and `> c`, in that order, with exit 1.
