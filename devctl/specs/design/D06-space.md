...
- R-CEEN-BLGK: When `LookupSpace` found an instance and `--no-backup` is not given, the `retire` step of `space destroy` MUST make exactly one call to `(host.Host).Sudo` with the step `retire` and exactly the arguments `opsctl` and `retire`, on a `host.Host` whose `Address` is the space's `Address` and whose `Deps` is `deps`, before any method of `clients` that terminates, releases, deletes, or changes anything other than the `apex` step's record delete; when that call returns a non-nil error destroy MUST return it unchanged, write no `retire` line and no later line, and make none of those later calls, so that the instance, the address, the space's records, its secrets, its backups, and its role are as they were and the diagnostic quotes the host's report; verified at least, for a space at `18.220.10.5` that does not hold the apex, by a fake `sudo opsctl retire` process that exits 1 writing to its standard output exactly the two lines `services: ok (crm, dashboard stopped)` and `crm: failed: /opt/crm/state/outbox: permission denied` and to its standard error exactly the line `sudo: unable to resolve host ip-172-31-5-9`, each followed by a newline, leaving stdout empty and writing to stderr exactly the five lines `devctl: retire: ssh ec2-user@18.220.10.5 sudo opsctl retire: exit status 1`, an empty line, `> services: ok (crm, dashboard stopped)`, `> crm: failed: /opt/crm/state/outbox: permission denied`, and `> sudo: unable to resolve host ip-172-31-5-9`, in that order, with exit 1.
