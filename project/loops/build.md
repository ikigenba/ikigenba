...
work and commit it. You do **not** decide completeness and you do **not** touch
`project/plan/STATUS.md`. Default to making progress; do not ask questions.
...
2. **Open gaps first.** If the `## Verify feedback` region lists open gaps, treat
   ...
   independent gate found unsatisfied last cycle. Each is tied to an `R-id` (or a
   named structural check) and the failing command/output — close **those**
...
3. **See what already exists** so the turn is idempotent (do not rebuild what is
   ...
   - run `go test ./...` from the repo root to read current failures;
   - re-run the brief's structural "Done when" commands (e.g. `test -x`,
     `bash -n`, the exact grep counts) to see which already pass.
4. **Do as much of the brief as cleanly fits this turn — ideally the whole
   ...
   - For a Go package: write id-tagged, genuinely-asserting tests — one
     `// R-XXXX-XXXX` comment on the test that proves that behavior (never a
     bare literal). Keep the tree buildable: `go build ./...` clean in each
     touched module. `gofmt -w` every Go file you touch.
   - For `bin/` shell tooling (untested by Convention — no `// R-` tags apply):
     satisfy the brief's structural done-bar commands exactly (e.g. make the
     script executable and `bash -n`-clean, hit the specified grep counts).
5. **Commit this turn's increment** (never an empty commit) with a phase-naming
   ...
   Leave `project/plan/STATUS.md` untouched — the marker stays `⬜`. Do not touch
   the brief. Always return `NEXT`.
...
- **Language / toolchain.** Go 1.26; modules wired by the repo-root `go.work` for
  ...
  build.
- **The green gate — "the suite is green" means `go test ./...` (from the repo
  root, workspace mode, covering every workspace module) exits 0.**
  Requirement-id tags live in `*_test.go` files.
- **Shell tooling is deliberately untested.** Scripts under `bin/` (`bump`,
  `ship`, `push-secrets`, …) carry no automated test tier — the former
...
- `message` — one short, plain sentence describing what happened, e.g.
  `Built bin/push-secrets + conforming .envrc files for Phase 48; committed.`
