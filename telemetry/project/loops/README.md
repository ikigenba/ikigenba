...
`telemetry/`, plus this overview. It lives beside the prompts it describes so it
...
`ralph` runs from the **service root** (`telemetry/`), so every workspace path
the prompts reference is service-root-relative. telemetry carries its **own**
`telemetry/go.work`, so `go` commands run from there resolve the in-repo
libraries through that workspace; `GOWORK=off` belongs to `bin/ship telemetry`
alone and the loop never sets it.
...
From `telemetry/`, all three must succeed:
...
**and** that test actually runs under `go test ./...`. telemetry has **no live
