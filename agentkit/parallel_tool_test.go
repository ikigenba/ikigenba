...
func TestToolAccessUsesValidatedConstructorArgumentsExactlyOnce(t *testing.T) {
	...
	typed := MustTool("typed", "", func(context.Context, parallelInput) (string, error) { return "ok", nil }, func(input parallelInput) Access {
	...
	orchestrator.prepareDispatch(ToolUse{ID: "4", Name: "raw", Input: json.RawMessage(`{"name":"raw"}`)}, false).run(context.Background())
...
func TestParallelDispatchCompletionOrderAndStableTranscript(t *testing.T) {
	...
	go func() { eventsDone <- drainStream(stream) }()
...
func TestCancelledSendCancelsAndWaitsForEveryTool(t *testing.T) {
	...
	transportCalls := 0
	conversation := newConversation(provider, successfulPhase15Client(&transportCalls), Config{Tools: []Tool{tool("a"), tool("b")}})
