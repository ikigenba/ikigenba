...
Do not flag a long function that is one cohesive phase: a state machine, an exhaustive switch over a token or message type, a table of cases, a decoder loop, or generated code. Do not flag a function that is long only because of straight-line initialization or per-error wrapping with no phase boundary to cut on, and do not flag a short function merely because it does two things. Do not flag an entrypoint that is already only wiring — a long parameter list or a long list of helper calls is not this smell. The test is whether the work lives in the function or behind calls: a body that is a straight sequence of calls, each followed only by its error return, is already wiring no matter how many blank lines separate the steps or how many locals hand one result to the next — that is the Spared shape below, not the Flagged one.
