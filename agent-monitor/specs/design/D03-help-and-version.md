...
`list`, `tree`, and `chat` can do, and which agent-monitor they have.
`D01-layout-and-run-seam` declares `Usage`, `ListUsage`, `TreeUsage`,
`ChatUsage`, and `Version` in `internal/cli`; this design fixes the values of
the four help
...
standard output. After `list`, `tree`, or `chat` it is an unknown option
(`D02`).
...
- R-KG4F-97HQ: When `args[0]` is `chat` and at least one element of `args[1:]` is byte-for-byte equal to `--help` or `-h`, `Run` MUST write exactly `ChatUsage` to `stdout`, write nothing to `stderr`, and return `ExitSuccess`, whatever the other elements of `args[1:]` are, so that `["chat", "--help"]`, `["chat", "-h"]`, `["chat", "claude", "7c2e9a41-3b0d-4f6e-9a57-2d8c1e0b5f93", "--help"]`, and `["chat", "bogus", "extra", "more", "more", "--bogus", "-h"]` all write `ChatUsage`.
