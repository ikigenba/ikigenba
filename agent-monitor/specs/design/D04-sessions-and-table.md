...
- R-W5YT-8BQJ: The package `internal/session` (import path `github.com/ikigenba/ikigenba/agent-monitor/internal/session`) MUST export exactly the identifiers `Status`, `StatusWorking`, `StatusIdle`, `StatusUnknown`, `Session`, `Table`, `ReadError`, `ErrNotJSON`, and `Lines`, as their own requirements declare them, plus the exported fields of `Session` and `ReadError` and the method `Error` of `*ReadError` those requirements declare, and no other exported identifier, method, field, constant, or variable.
