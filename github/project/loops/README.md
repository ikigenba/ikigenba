# github — the installed build loop

This folder holds the **gather → build → verify** loop that builds github one
...
`ralph` runs from the **service root** (`github/`) and re-invokes each prompt in a
...
`GOWORK=off` is this tree's declared GOWORK mode — it mirrors the production build
and proves the module resolves standalone through its `replace` directives. Every
gate test is fully offline.

Per `root project/design/D23.md` github has **hermetic** and **composed** layers in
the gate, a **manual** layer whose runbook is `project/github-verification.md`, and
no live layer. `verify` therefore also runs a **skip ban**:
`grep -rnE 't\.Skip(f|Now)?\(' --include='*_test.go' --exclude-dir=project .`
must print nothing, and any hit is an open gap.

### The ratchet and the one manual-layer id
...
must be empty, with two deterministic filters:

- the design-side `grep -v 'R-XXXX-XXXX'` drops the docs' literal format
  *placeholder*, which would otherwise surface as a phantom uncovered id;
- the remainder-side `grep -v '^R-DMUT-QF4A$'` drops the tree's single
  **expected out-of-gate id**. The expected set is exactly `{R-DMUT-QF4A}`: per
  design `D14` it is a **manual-layer** id, proven by the operator in the
  committed runbook `project/github-verification.md` (the positive `health` check
  plus the bad-key negative check that shows the positive one hit real GitHub),
  deliberately out of gate and unscheduled by any plan phase. Without the filter
  the ratchet would report it as a coverage regression forever.

Any *other* id in the remainder is a genuine regression. `R-DMUT-QF4A` is the only
id permitted to be absent from both the tag set and the pending-phase set.
