# audit — adversarially judge crm's requirement-id coverage, one Decision at a time

You run in a fresh, isolated context, one turn per invocation, as the single
step of an unattended audit loop over crm. `ralph` re-invokes this same prompt
each turn; `NEXT` wraps straight back to it. `ralph` runs from the service
root (`crm/`), so every path below is service-root-relative. All state lives
in the transient, gitignored `project/audit/` directory — never committed.
...
`# crm — Plan Status`. If it does not match, check
...
- **Build command:** `cd crm && go build ./...`.
- **Test command:** `cd crm && go test ./...` (full suite; the baseline gate
  and per-Decision runs use this). For an escalation, scope to the tagged
  test's package: `cd crm && go test ./<package>/... -run '<TestName>' -v`.
...
1. **Baseline gate.** Run, in order: `cd crm && go build ./...`,
   `cd crm && go vet ./...`, `cd crm && gofmt -l .` (must print nothing), and
   `cd crm && go test ./...`.
...
2. **Structural sweep** — five deterministic checks, run from `crm/`:
   ...
   b. **Duplicate assignment** — an id minted more than once across
      Decisions' Verification lists:
      ```
      grep -hoE '^- (R-[A-Z0-9]{4}-[A-Z0-9]{4})' project/design/D*.md | sed 's/^- //' | sort | uniq -d
      ```
      and an id tagged in more than one place:
      ```
   ...
   e. **Criteria trace** — confirm `project/design/INDEX.md` has a
      ...
      mapped id absent from the design set.
...
3. **Write the manifest** `project/audit/STATUS.md`:
   ...
   ... (one line per id-owning Decision from project/design/INDEX.md's
        ## Decisions section, in D-number order; skip structural Decisions
        that own no ids)
   ```
...
2. For every id in its `## Verification.` list:
   - Locate its tagged test(s):
     `grep -rn "R-XXXX-XXXX" --include='*_test.go' --exclude-dir=project crm`
   ...
   - **Static adversarial read** against the id's exact behavior statement:
     ...
     real substrate the Decision names (a real SQLite write/read-back, a real
     HTTP round-trip through the composition root), or a mock/fake where the
     design demands the real thing? Is it reachable under
     `cd crm && go test ./...` with no skip/gate?
   - Assign a verdict:
   ...
   - **Escalate to mutation only** when the static read suspects `weak` but
     ...
     2. In `$wt/crm`, apply the minimal mutation that violates the id's
     ...
     3. Run only the tagged test's package in the worktree, e.g.
        `cd "$wt/crm" && go test ./<package>/... -run '<TestName>' -v`.
...
3. **Wiring lens** — for every externally reachable surface this Decision
   declares (an HTTP route mounted through `Spec.Handlers`, an MCP tool
   registered in `internal/mcp`'s tool table, an event subscription), confirm
   at least one of its ids' tests reaches that surface through the
   composition root (`cmd/crm/main.go`'s `appkit.Main(appkit.Spec{...})`
   assembly, or the equivalent real assembly path) — never a handler/tool
   constructed directly by the test in isolation. A surface with no such test
   is an `unwired surface` finding: name the surface and the composition-root
   file (e.g. `cmd/crm/main.go`) that should mount it.
...
- baseline: green (`cd crm && go build ./... && go vet ./... && go test ./...` exit 0)
...
- criteria trace: pass | <unmapped criteria or ids>
...
- `message` — one short, plain sentence, e.g.
  `Audited D3: 3 covered (R-ASST-2B8C, R-ASST-4D1E, R-ASST-6F3G).`
