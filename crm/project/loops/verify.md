...
3. **Check coverage.**
   - **Code phase:** for **every** id listed under **Ids to cover**, confirm a
     ...
     `crm/etc/nginx.conf` and distinguishes the exact-match `= /srv/crm/` from the
     prefix `/srv/crm/`). **When uncertain that a test really asserts, treat the
     id as uncovered.**
   - **Structural / docs phase** (Ids to cover = "(none — structural phase)"):
     run the named content check instead — e.g. confirm
     `grep -i "no UI" crm/AGENTS.md` finds nothing, that `crm/AGENTS.md` states the
     landing-page truth, and that `crm/CLAUDE.md` still resolves to `crm/AGENTS.md`
     (the symlink is intact). The green suite plus the named check is the bar.
