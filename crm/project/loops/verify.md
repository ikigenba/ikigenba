...
2. **Run the full green suite** (from `crm/`), every command, and read the
...
3. **Confirm the tree is skip-free.** crm has **no live layer and no manual layer**, so no test
...
- Never delete a phase's `STATUS.md` line or `phase-NN.md` on anything short of a
  ...
  a skip is never acceptable green, and crm has no live layer, so there is
