...
root (`crm/`), so every path below is service-root-relative.
...
2. **Run the full suite:**
   ...
   - `cd crm && go test ./... -v` (all packages must pass; zero failures)
...
3. **For every id in the brief's "Ids to cover" list**, confirm a genuinely
   ...
   `go test ./...`:
   - Locate it: `grep -rn "R-XXXX-XXXX" --include='*_test.go' crm` (substitute
     the real id), scoped to files outside `project/`.
...
In addition to the brief's own ids, confirm crm's coverage has not regressed
overall:
...
Run from `crm/`. Output must be empty — every current design id is either
realized in a tagged, running test, or still owned by a pending phase. Any id
in the remainder is a coverage regression (its dropped tagged test is
recoverable from git history) and is itself an open gap for whichever phase
should own the fix — treat it as an open gap on the phase currently in the
brief.
...
- `message` — one short, plain sentence describing what happened, e.g.
  `Phase 21 passed: 5/5 ids covered, suite green, phase retired.` or
  `Phase 21 has 1 open gap (R-XXXX-XXXX uncovered); streak 1/3.`
