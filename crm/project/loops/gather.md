...
- **Test placement — co-locate, never phase-name.** A phase is one package, so
  its tests live in that package: `crm/internal/web/web_test.go`, `package web`,
  ...
  `crm/etc/nginx.conf` from disk and asserts over its content; place it in the
  `web` package (e.g. `crm/internal/web/nginx_test.go`) or another focused
...
- <any phase-specific check the phase's Done-when names, copied here verbatim —
  e.g. the docs purge's `grep -i "no UI" crm/AGENTS.md` finds nothing>
```
