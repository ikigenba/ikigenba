...
7. **Write `project/loops/brief.md`** to the exact schema below (overwrite any
   ...
   - the **files to touch**, the **dependency interface signatures** copied in (so
     ...
     deterministic conditions of the phase's own *Done when*, plus crm's green bar
     (below);
...
crm's green bar, from design's Conventions — copy it into the brief's **Done
...
no `SKIP`. crm has **no live layer and no manual layer**: no test file carries a
...
### Done bar
<the phase's own deterministic exit conditions, plus crm's green bar above;
...
- `message` — one short, plain sentence describing what happened, e.g.
  `Authored brief for Phase 19 (D21, 2 ids).`
