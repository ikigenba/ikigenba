...
- **Language / toolchain:** Go **1.26**, single module `module crm` rooted at
  `crm/`. Pure-Go SQLite driver `modernc.org/sqlite` (no cgo). The landing page
...
- **The chassis owns the server.** crm is `appkit.Main(appkit.Spec{…})`:
  `App:"crm"`, `Mount:"/srv/crm/"`, `Port:3100`, `MCP:true`, `Feed:"/feed"`
  ...
  identity gate, and the `/feed` mount are appkit's. main.go declares crm's
...
- **nginx is the sole trust boundary.** crm runs no token logic. nginx
  ...
  (the `crm/etc/nginx.conf` fragment), not a Go concern: the Go handler is mounted
  **ungated in-process**, exactly as `POST /mcp` relies on nginx for its bearer
  gate. crm binds `127.0.0.1` only.
...
`crm/internal/web/` — the handler, the embedded `*.html` template, and the
