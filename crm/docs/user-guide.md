...
*(Under the hood: `save` — a contact, company, deal, and task are all just things you save.)*
...
|---|---|---|
| Find or list things | "Find Wile E." · "Who's on the newsletter?" · "What deals are open?" | `search` |
| See everything about one thing | "Show me everything on the Coyote Pursuit deal." | `get` |
| Save or change something (incl. reminders) | "Add Wile E. as a subscriber." · "Bump him up to a lead." · "Remind me to book a discovery call." · "Mark that task done." | `save` |
| Remove something from view | "Get rid of that test task." | `delete` |
| Write down what happened | "Log that I demoed the rocket skates today." | `log` |
| Check you're connected | "Am I connected? Who am I logged in as?" | `health` |
...
**What happens:** Your assistant pulls up every contact tagged **newsletter** and hands you a tidy list. It's filtering on that exact tag — not guessing, not ranking — just "give me the people with this label, newest first." *(Under the hood: `search` over contacts, filtered to `tag: newsletter`.)*
...
**What happens:** Your assistant creates an **organization** named *Coyote Pursuit Industries* with that domain. Before it saves, it quietly checks that you don't already have a company with the same website or name — no accidental duplicates. *(Under the hood: `save` creating an organization `‹coyote-pursuit›`.)*
...
**What happens:** Your assistant adds an **email** entry to Wile E.'s timeline with your note, stamped with today's date. From now on, his history shows this first touch — and it'll keep growing as you work the relationship. *(Under the hood: `log` appending an `email` interaction on `‹wile-e›`.)*
...
**What happens:** Your assistant creates a **task** titled "Book discovery call," attaches it to Wile E. so you know who it's about, and sets the due date you described. It starts out **open**; it'll stay on your radar until you mark it done. *(Under the hood: `save` creating a task linked to `‹wile-e›`, with a due date.)*
...
**What happens:** Your assistant adds a **call** to Wile E.'s timeline, with your notes as the body and the time set to now. His timeline now reads like a little story: newsletter signup, first touch, and now a real discovery call. *(Under the hood: `log` adds a `call` interaction to `‹wile-e›`.)*
...
**What happens:** Your assistant moves Wile E. one rung up the relationship ladder, from **lead** to **opportunity**. Nothing else about his record changes — same email, same phone, same company, same newsletter tag. You changed one thing, so one thing moved. *(Under the hood: `save` updates the contact `‹wile-e›`, setting `lifecycle` to `opportunity`. On an update, only the fields you mention change; the rest are left exactly as they were.)*
...
**What happens:** Your assistant finds your open *"book a discovery call"* task and flips it to **done**. It also quietly stamps *when* it was finished, so your history of what-got-done-when stays accurate without you tracking a single date. *(Under the hood: `save` updates the task with `status: done`; `done_at` records the completion time.)*
...
**What happens:** Your assistant adds a **meeting** to the *deal's* timeline, with your note as the body and the time set to now. Notice you hung it on the deal, not on a person — and you never had to say "this is a deal, not a contact." You named what it's about; the agent worked out where the note belongs. *(Under the hood: `log`, kind `meeting`, on the deal `‹rocket-skates-deal›`.)*
...
**What happens:** Your assistant creates a new **contact** for Sam, links him to **Coyote Pursuit Industries**, records his title, and files his email as the primary one (tidied to lowercase, as always). Because this is a brand-new person, the CRM first makes sure nobody with that email is already on file — no accidental second Sam. *(Under the hood: `save` creating a contact, wired to the company `‹coyote-pursuit›`.)*
...
**What happens:** Your assistant updates the deal so its roster is now both people — **Wile E. Coyote** as champion and **Yosemite Sam** as decision-maker. You said "add Sam"; the agent re-sends the *whole* roster, both names, so nobody falls off. *(Under the hood: `save` updating the deal's participants — the list it sends becomes the complete roster.)*
...
**What happens:** Your assistant creates a **task** titled "Send proposal to Coyote Pursuit Industries," attaches it to the deal, and sets the due date. It starts out **open**, so it rides along on the deal until you close it. *(Under the hood: `save` creating a task linked to `‹rocket-skates-deal›`, with a due date.)*
...
**What happens:** Your assistant looks up the deal and returns its full **card** — the deal plus all its context, assembled in a single call. *(Under the hood: `get` on `‹rocket-skates-deal›`.)*
...
**What happens:** Your assistant adds a **call** to the deal's timeline, with your note about the discount written into the body. Notice you put this one on the *deal*, not on a person — the negotiation is about the deal as a whole, so that's where it belongs. *(Under the hood: `log`, kind `call`, subject `‹rocket-skates-deal›`.)*
...
**What happens:** Your assistant dutifully adds that **call** to **Daffy Duck's** timeline — exactly as asked. The trouble is, that note belongs on Wile E., not Daffy. *(Under the hood: `log`, kind `call`, subject `‹daffy-duck›` — the wrong subject.)*
...
**What happens:** Your assistant sets the deal to its closing stage — **won** — the outcome that ends the deal. (Every deal finishes one of two ways: **won** or **lost**.) Because a deal's *status* is derived from its stage, the status flips from **open** to **won** on its own — you named the stage; the status updated itself. *(Under the hood: `save`, updating the deal's `stage` to `won`; `status` becomes `won` on its own.)*
...
**What happens:** Your assistant moves Wile E.'s lifecycle from **opportunity** to **customer**, the top of the ladder. Nothing else about him changes — same email, same phone, same newsletter tag, same company. You just say "he's a customer now"; the agent knows that's a lifecycle move and picks the exact value. *(Under the hood: `save` on `‹wile-e›`, setting `lifecycle` to `customer`.)*
...
Sam earned the same upgrade — he's a contact on a won deal too. Just say "Sam's a customer now as well" and the agent promotes him the same way *(`save` on `‹yosemite-sam›`, `lifecycle` to `customer`)*. Two contacts, one closed deal, both now on the books as customers.
...
**What happens:** Your assistant lists every deal that hasn't been won or lost yet — the live pipeline, most-recently-touched first. *(Under the hood: `search` over deals, filtered to status **open**.)*
...
**What happens:** You get just the deals at the **proposal** stage. *(Under the hood: `search` over deals, filtered to stage **proposal**.)*
...
**What happens:** Your assistant returns the closed-won list — every deal where the boulder, for once, landed on the *target*. *(Under the hood: `search` over deals, filtered to status **won**.)*
...
**What happens:** Your assistant pulls the deal's complete timeline — every logged interaction about it, newest first — instead of just the recent slice a card carries. *(Under the hood: `search` over interactions, filtered to that deal as the subject.)*
...
**What happens:** Your assistant fetches the next batch, picking up exactly where the last one left off, and keeps going until you reach the start of the story — the coyote's very first ACME order. *(Under the hood: `search` continues from the last entry it saw.)*
...
**What happens:** Your assistant returns every contact whose lifecycle is **customer** — the people who, unlike a certain coyote, eventually caught what they were chasing. *(Under the hood: `search` over contacts, filtered to lifecycle **customer**.)*
...
**What happens:** Your assistant removes the task. It vanishes from the coyote's card and won't surface in searches — gone from every view you'll ever use. *(Under the hood: `delete` on the task.)*
...
So this section is a phrasebook. Same intent, twenty ways to say it — all of them work, because they all land on the same six tools underneath: `search`, `get`, `save`, `delete`, `log`, and `health`. Say it your way; the translation is on the house.
...
This is the opening move on almost any request. If you're not sure what exists yet, you're searching (the assistant reaches for `search`).
...
When you want the *full picture* of one specific record, that's a different move (the assistant reaches for `get`):
...
One verb covers organizations, contacts, deals, and tasks. Create, update, doesn't matter — it's all `save`. (Interactions are the exception; those get their own verb, below.)
...
The single most frequent thing you'll do: write down that something occurred. That's `log`, and it's append-only — a timeline, not a whiteboard.
...
`health` takes no input and just reports the identity the platform established for you — your owner email and client id (alongside the shared health envelope). It changes nothing. It's the verb you reach for once, right after setup, to prove the whole chain lit up — the handshake at the end of the wiring, not part of your daily work.

### The cheat sheet

| What you want | Say something like… | Verb underneath |
|---|---|---|
| See what exists / a list | "find," "who do we know," "show me all," "list the…" | `search` |
| The full story on one record | "open," "pull up," "everything on…" | `get` |
| Make or change a record | "add," "create," "set," "update," "tag," "remind me to…" | `save` |
| Write down what happened | "log," "note," "had a call/meeting/email…" | `log` |
| Make something go away | "delete," "remove," "drop," "get rid of…" | `delete` |
| Check who you're connected as | "who am I," "is this authenticated" | `health` |
