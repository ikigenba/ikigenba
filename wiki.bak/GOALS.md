...
| identity proof | `health` MCP tool |
...
Phase 1: `health`, `ingest_text`,
`ingest_url`, `search`, plus an ingest job-status read
(reuse agentkit's run/output verbs).
Later: `ask`, and whatever the dropbox consumer needs internally.
