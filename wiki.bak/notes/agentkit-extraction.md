...
Classified every file under `agent/internal/engine/**` plus the adjacent
`agent/internal/{runner,session,sandbox}`. Verified the import graph:

- **No engine package imports any non-engine agent package** (`grep` of
  `agent/internal/engine/**` for `agent/internal/{runner,session,sandbox,db,ids,…}`
...
- Only **four** non-engine agent files import the engine:
...
`engine/*` → `agentkit/*` and rewriting `agent/internal/engine/...` →
...
`replace agentkit => ../agentkit` in `agent/go.mod` and `wiki/go.mod`, and
`./agentkit` added to the root `go.work`.

**Package-path rule:** drop the `internal/engine/` segment. `agent/internal/engine/<pkg>`
...
|---|---|---|---|
| `agent/internal/engine/wire/` (all `*.go` incl. tests: `wire.go`, `decode.go`, `event.go`, `result.go`, `session.go`, `stdin_reader.go`, `text_block.go`, `thinking_block.go`, `tool_result_block.go`, `tool_use_block.go`, + all `*_test.go`) | `agentkit/wire/` | yes | stream-json NDJSON codec + `wire.Session` sink. Leaf package, zero agent deps. Move whole dir verbatim. |
| `agent/internal/engine/provider/provider.go`, `provider_test.go`, `clone_blocks_test.go`, `error_message_test.go` | `agentkit/provider/` | yes | provider-neutral `Client`/`Request`/`Event`/`Block`/`Error` abstraction. No agent deps. |
| `agent/internal/engine/provider/anthropic/` (`anthropic.go`, `anthropic_test.go`, `maxtokens_test.go`) | `agentkit/provider/anthropic/` | yes | Anthropic streaming client (pure net/http, CGO-free). Imports only `provider` + `trace`. |
| `agent/internal/engine/trace/trace.go` | `agentkit/trace/` | yes | optional debug tracer + key redaction. Zero agent deps. |
| `agent/internal/engine/model/` (`model.go`, `registry.go`, `model_test.go`, `registry_test.go`) | `agentkit/model/` | yes | model alias/provider resolution + pricing/context registry. **See Risk 4** — the alias map is agent-flavored but the package is generic; move as-is, tune data later. |
| `agent/internal/engine/schema/` (`schema.go`, `schema_test.go`) | `agentkit/schema/` | yes | minimal JSON-Schema subset for structured-output validation. Zero agent deps. wiki's ingest agent is freeform (`sch==nil`) so it won't use it, but it's generic and `agent` imports it. |
| `agent/internal/engine/tools/confine.go`, `confine_test.go`, `dispatch_confine_test.go` | `agentkit/tools/` | yes | path-confinement helpers used by `dispatch`. **See Risk 2** (reconcile with `sandbox.confine`). |
| `agent/internal/engine/tools/tools.go`, `tools_test.go` | `agentkit/tools/` | yes | tool registry (`All()`, `Select()`, `Descriptor`). **See Risk 3** — `All()` is a fixed set; wiki needs a per-consumer toolset. Move now, generalize in 1.2/4.1. |
| `agent/internal/engine/tools/dispatch.go` | `agentkit/tools/` | yes | routes a `tool_use` block to its impl. Generic. |
| `agent/internal/engine/tools/bash/` (`bash.go`, `bash_test.go`, `export_test.go`, `schema_test.go`) | `agentkit/tools/bash/` | yes | foreground `bash -c` tool. Pure-Go. |
| `agent/internal/engine/tools/read/` (`read.go`, `read_test.go`, `schema_test.go`) | `agentkit/tools/read/` | yes | read tool. |
| `agent/internal/engine/tools/write/` (`write.go`, `write_test.go`) | `agentkit/tools/write/` | yes | write tool. |
| `agent/internal/engine/tools/edit/` (`edit.go`, `edit_test.go`) | `agentkit/tools/edit/` | yes | edit tool. |
| `agent/internal/engine/tools/glob/` (`glob.go`, `glob_test.go`) | `agentkit/tools/glob/` | yes | glob tool. |
| `agent/internal/engine/tools/grep/` (`grep.go`, `grep_test.go`) | `agentkit/tools/grep/` | yes | grep tool. |
| `agent/internal/engine/agent/loop.go`, `loop_test.go` | `agentkit/agent/` | yes | the tool-use loop (`agent.Run`). The core generic machinery. Imports `model`, `provider`, `schema`, `tools`, `trace`, `wire` — all moving with it. |
| `agent/internal/engine/agent/prompt.go` | `agentkit/agent/` | **PARTIAL** | `FramingPrompt` const is **agent-specific** ("Ralph pattern", "leave deliverables as FILES", "no network"). **See Risk 5.** Move the *file* so `agent` compiles, but treat the constant as a default agent keeps overriding; wiki supplies its own system prompt (the schema doc) at the composition root. No code change needed — `agent.Run` already takes the prompt via `provider.Request.SystemPrompt`, not from this const. |

### Things that STAY in agent (do NOT move)

| source path | disposition | why |
|---|---|---|
| `agent/internal/runner/` (`runner.go`, `runner_test.go`, `runner_maxtokens_test.go`) | **STAYS in agent** | agent-specific async lifecycle: it knows `session.Store`, `session.Run`, `sandbox.Manager`, run-log files, TTL classification, `ANTHROPIC_API_KEY` read. PLAN 1.2 builds a **fresh greenfield** `agentkit/job` *informed by* this; agent migrates onto it only in Phase 7. After 1.1, `runner.go`'s imports are rewritten `agent/internal/engine/...` → `agentkit/...` but the file does not move. |
| `agent/internal/session/` (`model.go`, `service.go`, `store.go`, `*_test.go`) | **STAYS in agent** | agent's domain: sessions+runs state machine, single-flight gate, owner-scoping, SQLite store. wiki gets its **own** store (PLAN 3.1) behind agentkit's job-store interface. `service.go` keeps its one engine import (`model`), rewritten to `agentkit/model`. |
| `agent/internal/sandbox/` (`sandbox.go`, `sandbox_test.go`) | **STAYS in agent** | agent's per-session folder manager (`data/sandboxes/<id>/`, ULID-keyed). wiki has a different on-disk model (owner+collection tree, PLAN 3.1). **But** its `confine`/`resolveLongestExisting` logic is a duplicate of `tools/confine.go` — **see Risk 2**: the *reusable confinement primitive* should live in agentkit; the *Manager lifecycle* stays per-consumer. |
| `agent/internal/{db,ids,logging,mcp,server}`, `agent/cmd/**` | **STAYS in agent** | not in scope; agent chassis. (wiki clones the equivalents from ledger in Phase 2.) |
...
reproduce, distilled from the agent reference:
...
// serialized writer (agent relies on SQLite's single connection; wiki inherits
// the same db.Open).
...
- **`FlightKey` is the generalization of agent's `session_id`.** agent's
...
- **The single-flight invariant is enforced in `Store.Insert`, not in the
  runner.** That matches agent, where the atomicity comes from SQLite's single
  ...
  folding it into `Insert` keeps it one atomic write, which is what makes agent's
  gate race-free.)
- **`Job` is an interface, not a func, so the consumer can carry state** (wiki's
  ingest job carries the raw bytes + store + search reindex closure; agent's
...
- **Owner-scoping stays in the consumer.** agentkit's `Record` has no owner field;
  agent and wiki both layer "foreign-owned id reads as not-found" in their own
  `Store.Load` (as agent already does). This keeps the trust model
...
`runner.go` is welded to agent: `session.Store` (typed, not an interface),
...
agent's.** Mitigation: PLAN already says so — `agentkit/job` is **greenfield**
(1.2), the runner **stays** (1.1 only rewrites its engine imports), agent
...
- `agent/internal/engine/tools/confine.go` — `confinePath`, `effectiveSearchPath`,
  `resolveLongestExisting` (the **engine/tool-dispatch** copy; root may be `""` =
  unconfined).
- `agent/internal/sandbox/sandbox.go` — `confine`, `resolveLongestExisting` (the
...
2. `sandbox.Manager` **stays in agent** (it's agent's session-folder lifecycle),
...
**Recommended split:** move `tools` as-is in 1.1 (agent keeps using `All()`
...
### Risk 4 — "generic" packages carry agent-flavored data/assumptions
- `model/registry.go` + `model.go`: alias map (`opus`/`sonnet`/`haiku`),
  ...
  is agent's current pin. Move as-is; wiki reads model + cost ceiling from config
...
- `schema` package: built for agent's `--json-schema` structured-output path.
...
### Risk 5 — `agent.FramingPrompt` is agent-specific (the "Ralph pattern")
`agent/prompt.go`'s `FramingPrompt` const hardcodes agent's worldview ("autonomous
...
- Move `prompt.go` with the `agent` package (keeps agent compiling unchanged —
  agent's `runner` still references `agent.FramingPrompt`).
- Treat `FramingPrompt` as **agent's default**, not agentkit's contract. wiki
  ...
  **schema doc + integration-pass instructions** (PLAN 4.1). No code change in
  ...
  system prompt". (Optional later cleanup: rename to `AgentFramingPrompt` or move
  it back into agent's `runner` package, since nothing else in `agent` uses it —
...
agent domain packages (`db`, `ids`, `sandbox`, `session`) — those tests **stay
with the runner in agent** and just get their `agent/internal/engine/...` imports
rewritten to `agentkit/...`. They are the regression gate for Task 1.1
(acceptance: agent's suite passes unchanged). No cycle is introduced because
agent depending on agentkit is one-directional and agentkit never imports agent.
...
3. Global import rewrite **inside the moved tree and inside agent**:
   `agent/internal/engine/` → `agentkit/` (sed/`gofmt -r` over `*.go`).
4. `agent/go.mod`: add `require agentkit v0.0.0` + `replace agentkit => ../agentkit`.
...
6. Gate: `go build ./...` clean across the workspace; `go test ./...` in both
   `agentkit` and `agent` green (agent's behavior unchanged).
</content>
</invoke>
