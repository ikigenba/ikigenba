# eventplane — the installed build loop

This directory holds the unattended `gather → build → verify` loop that builds
the `eventplane` library one **pending phase** at a time from the spec in
...
`ralph` runs from the **service root** (`eventplane/`), so every workspace path
...
From `eventplane/`, in **workspace mode** via the repo-root `go.work` (this tree
deliberately does **not** set `GOWORK=off`), the suite is green when
`go test ./...` exits 0 with every package passing, `go vet ./...` exits 0, and
`gofmt -l .` prints nothing.

An id counts as **covered** only when it is named in a `// R-XXXX-XXXX` comment
on a test that genuinely asserts the behavior, co-located with the package it
exercises — cross-package end-to-end tests in `consumer/consumer_test.go`, never
a per-phase or root-level test file — running on the substrate its `Substrate:`
clause names, **and actually running under `go test ./...`**.

Per the suite testing contract (`root project/design/D23.md`), **every test in
this tree is hermetic**: eventplane builds no binary, so it has no composed
layer, no live layer, and no manual runbook. There is therefore no build-tag
carve-out here — a test held out of the default gate by a build tag, an env
flag, or a skip condition is unreachable and scores **uncovered**, and
`t.Skip`/`t.Skipf`/`t.SkipNow` may not appear in any test file in this tree at
all. `verify` enforces both with a source scan and a `--- SKIP` check.
