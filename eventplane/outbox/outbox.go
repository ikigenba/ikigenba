...
// lives outside the DB file so a file-level restore cannot roll it back; instead
// a box-level restore or rollback (`opsctl restore` / `opsctl rollback`) removes
// the sidecar, so the next boot mints a new epoch.
