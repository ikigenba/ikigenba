...
func toolDescriptors() []map[string]any {
	return []map[string]any{
		desc(tool("create"),
			"Create a named schedule. 'name' is the identity and the suffix of the emitted event type cron.<name> — lowercase letters, digits and hyphens only. 'expr' is a 5-field cron expression (minute hour day-of-month month day-of-week), evaluated in UTC; convert local time to UTC before setting. Operators: * , a number, lists a,b, ranges a-b, steps */n and a-b/n; day-of-week 0 or 7 is Sunday. The expression is validated here (fails loudly naming the bad field) and rejected if 'name' already exists.",
			obj(map[string]any{
				"name": descTyp("string", "schedule identity / cron.<name> suffix; [a-z0-9-]"),
				"expr": descTyp("string", "5-field cron, UTC, e.g. \"0 3 * * *\" (daily 03:00 UTC)"),
			}, "name", "expr")),
		desc(tool("list"),
			"List every schedule, ordered by name. Each entry returns {name, expr, created_at, updated_at, last_slot}. This is the authoritative view of which cron.<name> events exist (event reflection reports the shared payload shape; this reports the instances). Takes no inputs.",
			obj(map[string]any{})),
		desc(tool("get"),
		...
		desc(tool("delete"),
			...
			obj(map[string]any{"name": typ("string")}, "name")),
		desc(tool("health"),
			"Health + diagnostics for the cron service. Returns the fixed envelope (status, version, service, details) plus the authenticated caller's identity (owner_email, client_id) — the end-to-end auth-chain proof. Takes no inputs.",
			obj(map[string]any{})),
		desc(tool("reflection"),
			"Self-describe cron's place in the event graph: 'publishes' (the LIVE cron.<name> types, one per crontab row) and 'subscribes' (empty — cron is a producer). With no arguments returns the index {publishes:[{type,description}], subscribes:[]}. Pass 'event_type' (a published cron.<name>) for its {type, description, schema, example} detail.",
			obj(map[string]any{
