# cron — the installed build loop

This folder holds the `gather → build → verify` build loop as installed for
`cron/`, plus this overview. It lives beside the prompts it describes so it can
never document a different loop than the one on disk.
...
`ralph` runs from the **service root** (`cron/`), so every workspace path the
...
From `cron/`, all four must succeed:
...
**and** that test actually runs under `go test ./...`. cron has **no live
