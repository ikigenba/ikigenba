...
2. **Read exactly that one phase body** — `project/plan/phase-NN.md`. It names
   ...
   **Done when:** list of `R-XXXX-XXXX` ids (or a **structural** phase with no ids
   ...
   doctrine truth-statement is structural with a content check).
...
- **Test placement — co-locate, never phase-name.** A phase is one package, so
  its tests live in that package: `cron/internal/web/web_test.go`, `package web`,
  ...
  `cron/etc/nginx.conf` from disk and asserts over its content; place it in the
  `web` package (e.g. `cron/internal/web/nginx_test.go`) or another focused
