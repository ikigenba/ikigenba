# audit — adversarially judge whether cron's tagged tests prove the design
...
(`cron/`), so every path below is service-root-relative. Each turn answers a
stronger question than "does a tagged test exist?": for every minted
`R-XXXX-XXXX` id, does its tagged test actually prove the behavior cron's
...
## cron project conventions (inlined)

- **Build command:** `cd cron && go build ./...`.
- **Test command (the suite):** `cd cron && go test ./...`. "Green" means
...
- **Coverage convention:** an id counts as covered only when named in a
  ...
  **uncovered** however genuine its assertion — cron has no live layer and
  no manual layer, so no `*_test.go` file legitimately carries a
  `//go:build live` constraint and no skip anywhere is acceptable.
- **Composition root:** `cmd/cron/main.go` (the inline `cronSpec()
  appkit.Spec`), tested by `cmd/cron/main_test.go` — cron's single home for
  cross-package integration tests: the boot smoke (builds and runs the real
  binary against an `/opt/cron/`-shaped tree, checks `/health`), the landing
  route over the shipped `share/www` tree via `appkit/web`, the
  `POST /mcp` mount, read-from-disk assertions over `AGENTS.md`, and the
  shipped `cron/etc/nginx.conf` / `etc/manifest.env` content-assertions.
  This is cron's one composition root — the surface every externally
  reachable capability (the `GET /{$}` landing route, the bearer-gated
  `POST /mcp` tool table, the `cron:tick/<name>` event-plane publication)
  must be reached through for the wiring lens below.
...
1. Run the baseline gate exactly:
   ```
   cd cron && go build ./... && go vet ./... && test -z "$(gofmt -l .)" && go test ./...
   ```
   **Red baseline → refuse.** Write to `project/audit/REPORT.md`:
   ```
   # cron — Audit Report

   - baseline: RED — `cd cron && go test ./...` (or an earlier step) failed;
     see output below.
...
every externally reachable surface the Decision declares (the `GET /{$}`
landing route, the bearer-gated `POST /mcp` tool table, the
`cron:tick/<name>` event-plane publication), confirm at least one of its
ids' tests reaches that surface through `cmd/cron`'s composition root
(`appkit/web`'s real `Site`, the real `appkit/mcp` `ServeHTTP`, the real
outbox `Append`/`Publishes` chain driven by `internal/tick.Worker`) — not a
component the test constructs and drives directly. A surface no id's test
reaches this way is an `unwired surface` finding naming the surface and
`cmd/cron/main.go` (or the specific composition-root file) that should
mount it.
...
5. **Criteria trace** — every product success criterion has a line in
   ...
   `## Success criteria (outcomes)` heading) against the trace section's
   lines, and confirm every id cited in the trace section is in the design
   id set from check 4.
...
- **`covered`** — a tagged test exists, its assertion pins the
  **discriminating property** from the id's behavior statement (the "what
  ...
  real `appkit/mcp` transport, a real migrated `modernc.org/sqlite` DB via
  `internal/crontab.Store`, or `internal/tick.Worker.Fire` driven with an
  explicit clock — never a stub that accepts whatever it's handed where the
...
3. Run the tagged test's **package** in `$wt`:
   `cd "$wt/cron" && go test ./<pkg>/...` (the package containing the
...
`## Decisions` list, in Decision order; a structural Decision with no ids —
D5, D8, D12 — is omitted, it owns no id to audit.)
...
- baseline: green (`cd cron && go test ./...` exit 0)   [or the red-baseline refusal]
...
- unwired surface — <route/verb/subscription> (only when the wiring lens found
  one: no id's test reaches this declared surface through cmd/cron's
...
- `message` — one short, plain sentence, e.g.
  `Audited D14: 4 covered, 1 weak (R-PU4V-836L).`
