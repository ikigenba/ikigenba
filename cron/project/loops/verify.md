...
3. **Check coverage.**
   - **Code phase:** for **every** id listed under **Ids to cover**, confirm a
     ...
     `cron/etc/nginx.conf` and distinguishes the exact-match `= /srv/cron/` from
     the prefix `/srv/cron/`). **When uncertain that a test really asserts, treat
     the id as uncovered.**
   - **Structural / docs phase** (Ids to cover = "(none — structural phase)"):
     run the named content check instead — e.g. confirm
     `cron/cmd/cron/main.go`'s package-doc header states the landing-page truth
     (mentions a human web **landing page** under `/srv/cron/` alongside the MCP
     surface) and that `grep -rni "no UI" cron/` finds nothing. The green suite
     plus the named check is the bar.
