...
`ralph` runs from the **service root** (`bin/`, its working directory). This
tree has no module root of its own — its Go test package `bintest` rides the
repo-root `go.work`, which names it `./bin/bintest`, and workspace mode
resolves upward from `bin/` — so its toolchain commands
(`go build ./bintest/...`, `go test ./bintest/...`) and every workspace path
in the prompts are **service-root-relative**.
...
| **build** | `project/loops/brief.md` only | scripts under `bin/` + tests in `bintest/*_test.go` | this turn's increment | nothing |
| **verify** | `project/loops/brief.md`; the gate; mechanical id-set greps | `project/loops/brief.md` feedback region, or `project/loops/blocked.md` | the phase-retirement deletion | the phase's `STATUS.md` line + `phase-NN.md` + the brief, on a pass |
...
- `go build ./bintest/...` exits 0.
- `go test ./bintest/...` exits 0, no failures, **no `SKIP`**. **This tree is
...
- `gofmt -l bintest` prints nothing; `bash -n <script>` exits 0 for any
  script touched.
...
Tests live in `bintest/*_test.go`, **named for the script and behavior they
exercise**. `bin/` itself carries no tests, and `bintest` is the single
...
covered automatically in `bintest`. A phase realizing an untested-tier
...
design ids (project/design/D*.md, minus the R-XXXX-XXXX placeholder)
  − ( tagged-test ids in bintest/*_test.go ∪ pending-phase ids )
  = must be empty
```
...
`bintest` test is **hermetic**; the bash orchestration tier is **manual**.
...
— `project/` is read-only to it), delete `project/loops/blocked.md`, and
restart the loop.
