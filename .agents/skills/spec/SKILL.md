...
Enumerate ids over two file sets — the design documents, and the sub-project's test files as declared in `AGENTS.md` (the same declared set for every consumer, so all compute the identical gap):
...
A monorepo holds several sub-projects below one git root; each is independent and stays that way. A sub-project's `specs/` govern that sub-project's own directory and nothing else.
