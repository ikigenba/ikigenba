...
3. **Check the boundary.** Confirm no requirement reaches outside the project's own directory: no path into a sibling project, no build of one, no write into one, and no claim about the content of another project's output (see `draft.md`, "Depending on another project"). A dependency on a sibling appears only as its published interface plus a pinned version. If one does not, stop and say so.
