...
2. **Check the proofs.** Every requirement that states a fact about an external dependency (see the `spec` skill's `references/design-format.md`, "Never assume an external dependency") must be backed by an observation of the real thing before the check. Verification gathered at any earlier point counts; do not re-run it. If any such fact is still unproven, stop and gather the proof (or ask the user to) before checking.
