...
func runConsumer(ctx context.Context, rt *appkit.Router) error {
	logger := rt.Logger()
	// AGENT_CRON_FEED_URL is cron's loopback feed. The event plane bypasses
	// nginx, so this is a direct 127.0.0.1 address; the dev fallback is only for
	// `go run`/tests without env (cron's port is 3007).
	feedURL := config.EnvOr(os.Getenv, "AGENT_CRON_FEED_URL", "http://127.0.0.1:3007/feed")
	// AGENT_CRON_FROM is the first-subscription choice; tail by default so a fresh
	// agent only reacts to cron events fired from now on, not the entire backlog.
	from := config.EnvOr(os.Getenv, "AGENT_CRON_FROM", "tail")
...
func registerRoutes(rt *appkit.Router) error {
	...
	// AGENT_RUN_TTL bounds each run's wall-clock — the runaway-goroutine backstop
	// (§5.3). Parsed as a Go duration (e.g. "30m", "2h"). Read here at the domain
	// boundary, reusing appkit/config's env helper.
	runTTL, err := config.EnvOrDuration(os.Getenv, "AGENT_RUN_TTL", 30*time.Minute)
	if err != nil {
		return err
	}

	// The session data tree (sandboxes + run logs) lives alongside the db file —
	// the same AGENT_DB_PATH appkit resolved (default ./tmp/agent.db; /opt/agent/
	// data/agent.db on the box) — mirroring the on-box /opt/agent/data layout.
	dbPath := config.EnvOr(os.Getenv, "AGENT_DB_PATH", "./tmp/agent.db")
