...
func newTestHandler(t *testing.T) (*Handler, *sandbox.Manager) {
	...
	return NewHandler(svc, "1.2.3", "agent", nil), sb
}
...
func TestToolsListReturns14(t *testing.T) {
	...
	sort.Strings(got)
	want := []string{
		"ikigenba_agent_describe",
		"ikigenba_agent_health",
		"ikigenba_agent_session_cancel",
		"ikigenba_agent_session_clear_trigger",
		"ikigenba_agent_session_create",
		"ikigenba_agent_session_delete",
		"ikigenba_agent_session_fs_list",
		"ikigenba_agent_session_fs_read",
		"ikigenba_agent_session_get",
		"ikigenba_agent_session_list",
		"ikigenba_agent_session_output",
		"ikigenba_agent_session_run",
		"ikigenba_agent_session_set_trigger",
		"ikigenba_agent_session_update",
	}
	if len(got) != len(want) {
...
func TestSetAndClearTrigger(t *testing.T) {
	...
	created := call(t, h, "ikigenba_agent_session_create", map[string]any{
		"prompt": "hi",
	...
	set := call(t, h, "ikigenba_agent_session_set_trigger", map[string]any{
	...
	cleared := call(t, h, "ikigenba_agent_session_clear_trigger", map[string]any{
		"session_id": cv.SessionID,
	})
	if isError(cleared) {
...
func TestDescribe(t *testing.T) {
	h, _ := newTestHandler(t)
	res := call(t, h, "ikigenba_agent_describe", nil)
	if isError(res) {
		t.Fatalf("ikigenba_agent_describe returned isError: %+v", res)
	}
	txt := resultText(t, res)
	if len(txt) == 0 {
		t.Fatal("ikigenba_agent_describe returned empty text")
	}
	// Sanity: it should actually describe the lifecycle entry points, not just
	// be non-empty.
	for _, want := range []string{"session", "ikigenba_agent_session_create", "ikigenba_agent_session_run"} {
		if !strings.Contains(txt, want) {
			t.Fatalf("ikigenba_agent_describe text missing %q:\n%s", want, txt)
		}
	}
}
...
func TestInitializeIncludesInstructions(t *testing.T) {
	...
	if !strings.Contains(resp.Result.Instructions, "ikigenba_agent_describe") {
...
func TestHealth(t *testing.T) {
	h, _ := newTestHandler(t)
	res := call(t, h, "ikigenba_agent_health", nil)
	var out struct {
	...
	if out.Status != "ok" || out.Version != "1.2.3" || out.Service != "agent" {
...
func createSession(t *testing.T, h *Handler) string {
	t.Helper()
	res := call(t, h, "ikigenba_agent_session_create", map[string]any{
		"prompt": "do a thing",
		"config": map[string]any{"model": "haiku"},
		"name":   "test",
	})
...
func TestDispatchRoundtrip(t *testing.T) {
	...
	getRes := call(t, h, "ikigenba_agent_session_get", map[string]any{"session_id": id})
	...
	// list includes it.
	listRes := call(t, h, "ikigenba_agent_session_list", nil)
	var listOut struct {
	...
	fsRes := call(t, h, "ikigenba_agent_session_fs_list", map[string]any{"session_id": id})
	var fsOut struct {
	...
	runRes := call(t, h, "ikigenba_agent_session_run", map[string]any{"session_id": id})
	var runOut struct {
		Status string `json:"status"`
	}
	...
	readRes := call(t, h, "ikigenba_agent_session_fs_read", map[string]any{"session_id": id, "path": "hello.txt"})
...
func TestErrorMapping(t *testing.T) {
	...
	res := call(t, h, "ikigenba_agent_session_get", map[string]any{"session_id": "nope"})
	if !isError(res) {
	...
	if r := call(t, h, "ikigenba_agent_session_run", map[string]any{"session_id": id}); isError(r) {
		t.Fatalf("first run: unexpected isError %+v", r)
	}
	busy := call(t, h, "ikigenba_agent_session_run", map[string]any{"session_id": id})
	...
	upd := call(t, h, "ikigenba_agent_session_update", map[string]any{
		"session_id": id, "prompt": "x", "config": map[string]any{"model": "haiku"},
	})
	if !isError(upd) {
		t.Fatalf("update while running: want isError, got %+v", upd)
	}
	del := call(t, h, "ikigenba_agent_session_delete", map[string]any{"session_id": id})
	if !isError(del) {
		t.Fatalf("delete while running: want isError, got %+v", del)
	}

	// fs_read path escape -> isError.
	esc := call(t, h, "ikigenba_agent_session_fs_read", map[string]any{"session_id": id, "path": "../escape"})
	if !isError(esc) {
