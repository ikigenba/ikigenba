...
// real box uses root="/opt"; tests use a temp dir. The stable bin/run path, the
// active etc/current config path, and the never-touched state/<app>.db path are
// computed here so every verb shares one definition.
//
