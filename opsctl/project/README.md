# opsctl/project — workspace layout

Everything the opsctl service needs to be **designed, planned, and built**
...
root** (`opsctl/`), which is also the directory the `ralph` build loop runs
from.
...
| `plan/` | `README.md` (spine) + `STATUS.md` (the manifest — the `Next phase` counter plus the only home of each pending phase's `⬜` marker) + `phase-NN.md` (one per pending phase) | `/plan-mode` (appends pending phases; completion is deletion by the build loop) |
...
- **verify** — the independent gate and only prompt that mutates `STATUS.md`.
  ...
  commit the deletion; gap → leave it `⬜`. Either way it deletes
