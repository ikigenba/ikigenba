...
- R-IZ05-BFFI: A request carrying identity whose path is `/` or `/widgets` and whose method is `GET` or `HEAD`, a request `Handler` answers with status 404 or 405, and a request `Handler` answers with the missing-identity 500, MUST each leave the store unchanged: the slice `s.All()` returns before the request and the slice it returns after the request MUST be equal element for element and in the same order.
