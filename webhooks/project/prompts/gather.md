...
a fresh, self-contained `project/prompts/brief.md`. **You write no code, run no
...
6. **Write `project/prompts/brief.md`** to the schema below (overwrite any
   existing one), then end your final message with exactly:
   `{"status": "NEXT", "message": "Brief written for phase NN."}`

## `project/prompts/brief.md` schema (emit exactly these sections)
...
<one bare R-XXXX-XXXX per line, so
 `grep -oE 'R-[A-Z0-9]{4}-[A-Z0-9]{4}' project/prompts/brief.md` enumerates them;
