...
genuinely-asserting, actually-running tagged test, with the suite green? You never
halt and never advance a phase on a gap.

## Procedure

1. **Read the brief.** If `project/prompts/brief.md` is missing or empty, end with
   `{"status": "NEXT", "message": "No brief to verify."}`.
...
3. **Confirm coverage for every id.** Enumerate the ids:
   ...
   **uncovered**:
   - a tagged test that **reported `SKIP`** in this run — a skipped requirement test
     is a gap, never acceptable green;
   ...
   plus any named smoke in its done bar instead of ids.

4. **Judge and act:**
   - **PASS** — suite green **and** every id covered (or structural + green): flip
     **only this phase's** marker `⬜ → ✅` on the exact `status_line` recorded in
     the brief, leaving every other `STATUS.md` line byte-for-byte unchanged. Commit
     just that one-line flip:
   ...
   - **GAP** — anything short of green + full coverage: leave the marker `⬜`, change
     no source, make no commit. The loop will re-gather and re-attack this phase.

5. **Always, as the final step** (both PASS and GAP):
...
6. End your final message with exactly:
   `{"status": "NEXT", "message": "Phase NN <passed|left ⬜: short reason>."}`

## Boundaries

- Never write or fix production code; never edit a test. If the phase has a gap,
  your job is only to leave the marker `⬜` — build will re-attack it next cycle.
