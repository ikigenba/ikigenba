...
- **`cmd/agent-repl`** — `main` wires the real values: `os.Args[1:]`,
  ...
  `time.Now`, `os.Getwd()`, and a channel fed by `signal.Notify` for
...
an `httptest` provider, and checks its output and the log file it wrote. Like
oauth's D01, this requirement depends on D2–D6 having landed and belongs in a
final phase.
