...
- R-W3NJ-9HJK: `Run` MUST build `session.Config` from `Options` and `Deps` field for field — `Provider`, `Model`, `Wire`, `Auth`, `AuthFile`, `BaseURL`, and `Settings` from `Options`; `Home`, `Getenv`, and `Root` from `Deps` — and pass the log it created.
