...
- R-VK55-55OG: For each host with at least one cataloged offering whose `AuthModes` contains `agentkit.AuthModeOAuth`, `help.Providers()` MUST follow that host's line with a line of 15 spaces, `auth=oauth` left-justified in 14 columns, and `(auth_file=~/.agent-repl/<host>-auth.json)`, and MUST emit no such line for any other host.
