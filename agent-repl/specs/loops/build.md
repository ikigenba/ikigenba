...
4. **Implement.** Write the code and, for each phase id, a test tagged with that id (the id in a comment, or in a string where a comment cannot sit) that genuinely asserts the behavior.
   ...
   - **Complete replacements.** When the brief removes ids, retire what they described: delete or untag their tests, remove or reshape the code, and migrate every caller the brief names. Do not leave the old shape beside the new — no alias, adapter, or forwarding layer whose only purpose is to keep superseded structure alive. Previously built code has no claim to preservation beyond what the current design requires.
   ...
   - After any change, **every** active requirement id — every id not listed for removal in this brief — must still be tagged, present, and green. Never delete or weaken a test for an active id to make a refactor pass; the only tests that go are those of the ids the brief removes.
...
Always report `NEXT`. Build never ends the run: it cannot judge whether the plan is finished, so `DONE` is never a correct answer from build.

## Report

End with `NEXT` on its own line and a one-sentence message. When the harness asks for a structured status field, set it to `NEXT` — never `DONE`, never `CONTINUE` — regardless of what the message says; ralph reads the field, not the prose. Finishing the phase, every gate green, or nothing left to build are all `NEXT`: they mean verify runs next, not that the run ends.
