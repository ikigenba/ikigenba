# ledger/project — workspace layout

Everything the ledger service needs to be **designed, planned, and built**
...
root** (`ledger/`), which is also the directory the `ralph` build loop runs
from.
...
| `plan/` | `README.md` (spine) + `STATUS.md` (the manifest — the `Next phase` counter and the only home of each pending phase's `⬜` marker) + `phase-NN.md` (one per **pending** phase) | `/plan-mode` (a work queue: completion is deletion — the build loop removes a finished phase's line and body file; history lives in git) |
...
- **verify** — the independent gate and only prompt that mutates `STATUS.md`.
  ...
  file, and commit; gap → leave the line and its `⬜` marker untouched. Either
