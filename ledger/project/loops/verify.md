...
3. **Check coverage.**
   - **Code phase:** for **every** id listed under **Ids to cover**, confirm a
     ...
     `ledger/etc/nginx.conf` and distinguishes the exact-match `= /srv/ledger/` from
     the prefix `/srv/ledger/`). **When uncertain that a test really asserts, treat
     the id as uncovered.**
   - **Structural / docs phase** (Ids to cover = "(none — structural phase)"):
     run the named content check instead — e.g. confirm
     `grep -i "no UI" ledger/AGENTS.md` finds nothing, that `ledger/AGENTS.md`
     states the landing-page truth, and that `ledger/CLAUDE.md` still resolves to
     `ledger/AGENTS.md` (the symlink is intact). The green suite plus the named
     check is the bar.
