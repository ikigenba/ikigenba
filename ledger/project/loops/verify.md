...
2. **Run the full green suite** (from `ledger/`), every command, and read the
   ...
   The last command is ledger's **isolated build check**: it mirrors the
   production build's module resolution (`bin/ship ledger` forces `GOWORK=off`)
   and runs no test. It is a declared fact of ledger's testing surface and part
   of the green bar here, so a change that only resolves through the workspace
   fails the gate rather than the release.

   Any non-pass (build/vet error, `gofmt -l .` prints a file, a failing or
   **`SKIP`ped** test, a non-zero `GOWORK=off go build ./...`) is a gap. **A skipped `R-XXXX-XXXX`-tagged test is a gap,
...
3. **Confirm the tree is skip-free.** ledger has **no live layer and no manual layer**, so no test
...
- Never delete a phase's `STATUS.md` line or `phase-NN.md` on anything short of a
  ...
  green, and ledger has no live layer, so there is no file in which a skip is
  permitted.
...
- `message` — one short, plain sentence describing what happened, e.g.
  ...
  `Phase 19 blocked: second stall, wrote blocked.md.`
