...
- **Test placement — co-locate, never phase-name.** A phase is one package, so
  its tests live in that package: `ledger/internal/web/web_test.go`, `package web`,
  ...
  `ledger/etc/nginx.conf` from disk and asserts over its content; place it in the
  `web` package (e.g. `ledger/internal/web/nginx_test.go`) or another focused
