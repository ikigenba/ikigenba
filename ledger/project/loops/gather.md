...
of an unattended `gather → build → verify` loop that builds ledger one phase at a
time. `ralph` runs from the service root (`ledger/`), so every path below is
...
2. **Find the next unstarted phase.** Run:
   ...
   - Otherwise, read the phase number `NN` from the matched line (e.g.
     `- Phase 19 ⬜ realizes R-O1AD-MRKW, R-O2IA-0JBL — …` → `NN = 19`).
...
7. **Write `project/loops/brief.md`** to the exact schema below (overwrite any
   ...
   - the **files to touch**, the **dependency interface signatures** copied in (so
     ...
     deterministic conditions of the phase's own *Done when*, plus ledger's green bar
     (below);
...
ledger's green bar, from design's Conventions — copy it into the brief's **Done
bar** verbatim, alongside the phase's own conditions:
...
All five succeed with zero failures. The last is ledger's **isolated build
check**: it mirrors the production build's module resolution (`bin/ship ledger`
forces `GOWORK=off`) and runs no test, and it is part of the green bar here. Requirement-id tags live in files matched
...
no `SKIP`. ledger has **no live layer and no manual layer**: no test file carries a
...
### Done bar
<the phase's own deterministic exit conditions, plus ledger's green bar above;
each id covered by a co-located, genuinely-asserting `// R-id` test that runs
under `go test ./...` with no SKIP; a structural phase names its
