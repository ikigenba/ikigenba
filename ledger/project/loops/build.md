...
3. **Do one increment of the remaining work.** Build the package(s) / artifact
   ...
   **code** phase, write id-tagged, genuinely-asserting tests: each Verification
   ...
   `ledger/internal/web/web_test.go`, `package web`, named for the behavior — never
   in a root-level or `phaseNN_test.go` file. The nginx-fragment test reads
   `ledger/etc/nginx.conf` from disk and asserts over its content; keep it in a
   focused package (e.g. `ledger/internal/web`). For a **docs/structural** phase,
   ...
   - **Composition root.** `cmd/ledger/main.go` is grown incrementally (e.g. adding
...
4. **Keep the suite green for what you've written** and format:
   ...
   purge's `grep -i "no UI" ledger/AGENTS.md` finding nothing).
...
- **"The suite is green"** means all of: `cd ledger && go build ./...`,
  ...
  and `cd ledger && go test ./...` succeed with zero
  failures.
- **No schema change for the landing page.** It touches no SQLite and adds **no**
  ...
  ledger <name>` — but this work needs none.)
