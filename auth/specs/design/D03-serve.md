...
- R-4KCJ-48RH: With a whole configuration, `cli.Run` MUST call `store.Open(Process.DBSource, Process.Rand)` and MUST NOT create the loopback listener until that call returns successfully, so the store is open before any address is listened on. This ordering MUST hold without any reliance on `store.Open` blocking or on timing: when `Process.DBSource` names a source that `store.Open` rejects, `cli.Run` MUST accept no connection on `127.0.0.1:<PORT>` and MUST return 1 (the error path of R-IN2K-GOI6); when the source opens, the listener MUST be created only afterward, serving the opened `*store.Store`.
...
- R-4O08-9JZK: `cli.Run` MUST construct the Google client via `google.NewClient(GOOGLE_CLIENT_ID, GOOGLE_CLIENT_SECRET, WORKSPACE_DOMAIN, Process.OIDCIssuer)`, then construct the server via `server.New` with a `server.Config` whose `Store` is the opened `*store.Store`, `Google` is that client, `Now` is `Process.Now`, `Rand` is `Process.Rand`, `Stderr` is `Process.Stderr`, and `WorkspaceDomain` is `WORKSPACE_DOMAIN`, then run it via `Server.Serve` on `127.0.0.1:<PORT>` and stop it via `Server.Shutdown`, making no network call before it listens. This wiring MUST be observable through the running listener, not only at construction: with `Process.OIDCIssuer` set to a reachable loopback OIDC issuer, `Process.Rand` a deterministic reader, and `Process.Now` a fixed clock, a `GET /login/google` served by `cli.Run` MUST return `302` whose `Location` is addressed to that issuer's discovered `authorization_endpoint` and whose `state` and `code_challenge` are derived from `Process.Rand`, so the injected client, issuer, and randomness are proven to reach the server; the sign-in handler's own contract is D05's (R-IGLN-K2W2, R-KY4E-8B7G), not this requirement's.
