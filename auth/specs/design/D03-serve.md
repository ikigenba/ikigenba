...
socket on the same terms; `127.0.0.1:3001` is the origin whose callback is
registered on the OAuth client for development (D05).
...
- R-N7J3-KX6N: A `Run` that calls `server.Serve` MUST write nothing to `Stdout`, and MUST write nothing to `Stderr` other than what the server writes through the writer of R-N07P-AAQH and, when `Serve` returns a non-nil error, the one line R-N8QZ-YOXC states.
...
- R-N9YW-CGO1: `Run` MUST NOT let two calls to `Stderr.Write` be in progress at the same time, those made through the writer of R-N07P-AAQH included, so that a `Stderr` that is not safe for concurrent use is never written concurrently.
