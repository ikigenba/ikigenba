...
- R-W3BJ-0PN1: A running `auth` server MUST be self-contained in its executable: it MUST serve its HTML, JavaScript, and CSS from assets embedded in the binary, MUST depend on no shared library at run time, and MUST open no file at run time other than its SQLite database at `state/auth.db` and the files SQLite keeps beside it; the inherited file descriptor 3 and the `NOTIFY_SOCKET` datagram socket are not files it opens.
