...
D06's. Server construction and Google-config validation are D03's. S7's routing
...
- R-IJ1G-BMDG: On a successful sign-in the response MUST set the `SessionCookieName` cookie to the created session's opaque id with attributes `Secure`, `HttpOnly`, and `SameSite=Lax`; on a space it MUST also set `Domain=<space>`, and run locally it MUST set no `Domain`.
- R-IK9C-PE45: On logout the response MUST clear the `SessionCookieName` cookie with an empty value, `Max-Age=0`, and the same attributes as the login cookie (`Secure`, `HttpOnly`, `SameSite=Lax`, `Domain=<space>` on a space and no `Domain` locally).
...
- R-J2JU-FY8K: When a matched-state callback's token exchange fails or Google is unreachable, auth MUST respond `502` with `Content-Type: text/plain; charset=utf-8` and a single line of body, MUST write the underlying error to `Process.Stderr`, and MUST create no user, session, or cookie.
