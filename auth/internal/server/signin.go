...
func (s *Server) handleRoot(w http.ResponseWriter, r *http.Request) {
	identity, signedIn := s.sessionIdentity(r)
	if !signedIn {
	...
	if err := s.renderTokenRows(w, identity.UserID); err != nil {
		return
	}
...
func (s *Server) writeDiagnostic(err error) {
...
func (s *Server) handleLoginGoogleCallback(w http.ResponseWriter, r *http.Request) {
	...
	if r.URL.Query().Get("error") == "access_denied" {
		if stateValue != "" {
			_, _ = s.st.ConsumeLoginState(stateValue)
		}
		writeSignInPage(w, "")
		return
	}
...
func (s *Server) sessionIdentity(r *http.Request) (store.Identity, bool) {
