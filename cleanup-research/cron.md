...
## Other stale info
- cron/project/design/design.md:57-58 — Fixed verb set listed as `serve`/`version`/`manifest`/`migrate`/`backup`/`restore`. This is superseded: CLAUDE.md states the fixed verbs are `serve`/`version`/`manifest`/`migrate`/`schema`, and "Backup and restore are NOT binary verbs — they are box-level operations owned by opsctl." Doc omits `schema` and wrongly lists backup/restore. (superseded verb set)
