...
## Other stale info
- opsctl/cmd/opsctl/main.go:79 — rollback help synopsis `opsctl rollback <app> [version]`; explicit target version is no longer supported (rollback.go:100 rejects it: "use -N snapshot recency"). Should read `[-N]`. (superseded verb args) ✅ **DONE 2026-07-03** (changed to `[-N]`)
- opsctl/cmd/opsctl/main.go:278 — same stale `usage: opsctl rollback <app> [version]` error string; recovery model is now S3-snapshot recency (`-N`), not explicit version. (superseded verb args) ✅ **DONE 2026-07-03** (error string changed to `[-N]`)
- opsctl/project/README.md:15 — "End-user documentation for this service lives in `opsctl/docs/`" but no `opsctl/docs/` directory exists. (dead path) ✅ **DONE 2026-07-03** (the docs/ pointer sentence was removed from opsctl's README in the suite-wide scaffold/docs cleanup)
