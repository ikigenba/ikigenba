...
- scripts/project/design/design.md:60 — Lists the fixed verb set as `serve`/`version`/`manifest`/`migrate`/`backup`/`restore`. Superseded: current set is `serve`/`version`/`manifest`/`migrate`/`schema` (CLAUDE.md; cmd/scripts/main.go:6). `backup`/`restore` are box-level opsctl operations, not binary verbs (and this service's own D09.md:76 states exactly that). (superseded verb set)
- scripts/project/design/design.md:19 — Points to `scripts/project/notes/ARCHITECTURE.md` and `scripts/project/notes/PLAN.md` as owning the existing domain. `project/notes/` does not exist. (dead path)
