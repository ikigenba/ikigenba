...
## Other stale info
- webhooks/project/design/D01.md:50 — Lists the chassis "fixed verbs" as `serve|version|manifest|migrate|schema|backup|restore`. `backup`/`restore` were removed from appkit (appkit/appkit.go:214-227 dispatches only serve|version|manifest|migrate|schema; appkit_test.go `TestDispatch_BackupRestoreRemovedAndSpecHasNoHooks` asserts they 404). Backup/restore are now box-level opsctl operations, not binary verbs. (superseded verb set / removed feature)
