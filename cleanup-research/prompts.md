...
- prompts/bin/start:20 — Calls `bin/build`, which no longer exists (only secrets/start/stop/teardown in bin/); main.go:16 explicitly notes the "deleted bin/build run-wrapper". Dead script reference — the `if [[ ! -x "$BIN" ]]; then bin/build; fi` branch would fail. (dead path)
