...
## Other stale info
- crm/AGENTS.md:30 — dead path: "The full design is in `project/notes/PLAN.md`". No `crm/project/notes/` dir exists; design now lives in `crm/project/design/`. (dead file path)
- crm/project/design/design.md:18 — same dead path `crm/project/notes/PLAN.md` cited as owner of the CRM domain. (dead file path)
- crm/bin/start:16 — calls `bin/build`, which does not exist (crm/bin/ holds only start+stop; main.go:12 confirms the bin/build run-wrapper was deleted). Local start path is broken/stale; build now goes through the Makefile. (dead script reference)
