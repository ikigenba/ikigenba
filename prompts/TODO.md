...
drives it through MCP (`session_*`). The friction is concentrated at the
...
- The **path convention**: relative paths resolve against the sandbox root; an
  absolute path outside the root is rejected by confinement. (The `describe`
...
- The **deliverable contract**: the answer is the file(s) it leaves behind, not
  its final chat message. (We already tell callers this in `describe`; the
...
**Problem.** Today the only way to read a deliverable is `session_fs_read`,
which streams the file back **through the MCP tool result** as text. We already
hit the ceiling: the go-vs-rust run's `session_output` log (~72 KB) blew the
...
- **`max_tokens` semantics are now "model max by default."** Good, but document
  it (in `describe` config notes) so callers know an unset value isn't a
