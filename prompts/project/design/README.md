...
- **Published agentkit**: `github.com/ikigenba/agentkit v0.6.0` — the external dependency (currently pinned at `v0.2.1`; D1 bumps to `v0.6.0`, the release carrying the model catalog, typed-credential constructors, the OpenRouter provider, and consumer-owned cost resolution — research §2, §5). The `v0.6.0` tag is published; both local dev and the production build (`GOWORK=off`) resolve it from the module cache.
