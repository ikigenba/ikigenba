...
## The version plane (a prompt definition is a git tree)

A prompt's definition is **not** in the database: it is a git repository held by the version plane, keyed `prompts/<name-key>` where the key is a slug of the prompt's name that prompts keeps globally unique (D51). Every repository has the same three addressable paths — `prompt.md`, `config.json`, and an optional `system.md` — plus whatever else its author puts beside them; the `prompts` row keeps only metadata, and its three content columns are retired once seeding has moved them (D54, D59). One injected client, `internal/version`, is prompts' entire knowledge of the plane and the only place the service name `repos` appears outside the composition root (D52). `create`, `update`, `import`, and rename each become one batched commit; `delete` archives rather than deletes (D53). Versioning stays **ambient**: nobody performs a git action to get history, and the same definitions are cloneable and pushable from a laptop.

## Run durability, the clone, and the two loaders

A run is a **durable, self-contained record**. Its artifacts live in one directory per run under the service's durable state tree — `<stateDir>/runs/<run_id>/` holding `input/` (the triggering `event.json`, nothing else), `output.jsonl`, and `sandbox/` — which survives restarts and is never reclaimed automatically (D39).

The sandbox **is a git clone** of the prompt's own repository. At spawn prompts resolves `main`'s head, records it on the run row as `definition_sha`, mints a short-lived per-repository run token, clones through the plane's git door, and checks out a branch `ikigenba/run-<run_id>` at that exact commit (D55, D56). The agent works in that clone with its ordinary tools, may commit and push its branch, and merges only if the prompt's own instructions say so — through the plane's `merge` verb, which it reaches as an ordinary suite MCP peer. The framing prompt states all of that, including the real branch and sha (D57).

That gives the two named loaders returning one shape, `Executed`: `LoadFromPrompt` reads the live definition (the row's metadata plus the plane's `main` tip) and is called at spawn and by `get`; `LoadFromRun` reads the **pinned commit out of the run's own clone** and backs every read of a past run (D40). The rule they encode is that **the run path reads neither the `prompts` table nor the version plane** — everything a past run needs is inside its own directory, so a run stays readable after its prompt is deleted and its repository archived, and while the plane is down. Run-side ownership is scoped off `runs.owner_id`, which is why `RunList` does not precondition on the prompt existing.

Deleting a prompt **does not cascade**: it removes the prompt row and its triggers and archives its repository, and its runs survive intact, listable and readable. Run history is removed only by `run_delete`, which takes one run and removes its `calls` rows, its `runs` row, and its directory — the clone with it (D41).
