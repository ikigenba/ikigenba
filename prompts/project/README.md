...
| `plan/` | `README.md` | the spine (static rules) + `STATUS.md` (the `Next phase` counter and the manifest — the only home of each pending phase's `⬜` marker) + `phase-NN.md` (one per **pending** phase) | plan authority (queue: appends new phases; the build loop deletes completed ones) |
...
that, on a pass, deletes that phase's `STATUS.md` line and its `phase-NN.md` —
there is no `✅` marker; done is gone. The loop stops when `gather` finds no `⬜`
phase (the queue is empty). The prompt bodies under `project/loops/` are the
authority on their own contract.
