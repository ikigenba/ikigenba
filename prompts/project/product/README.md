...
- **Suite tools reach the in-run agent directly.** The in-run agent can use every other suite service on the owner's behalf: at run start prompts attaches the account's reachable services to the conversation, and their tools are callable immediately, with no loading step. The cost of that directness is that a run carries every attached service's tool definitions for its whole life, and that a service which fails to answer at run start fails the run rather than being quietly skipped. Both are accepted for now; making attachment tolerant of a down service is a change owed by the shared agent library, not by prompts.

- **A run is a durable, self-contained record.** Once a run has executed, what it ran and what it produced survive indefinitely — across service restarts, and across the deletion of the prompt it came from. The owner can always go back to a run and see the exact prompt text and configuration that run used, which is not necessarily what the prompt says today, plus its output and the files it left behind. Deleting a prompt removes only the prompt; its runs remain listed, readable, and complete.
...
**A run reaches suite tools directly.** The in-run agent starts each run with the account's services already attached and their tools immediately callable. An event-triggered run follows its event's identifiers straight to the right service's tools. Which services a run may reach is unchanged: all of them except prompts itself, on the owner's behalf. A service that does not answer when the run starts causes the run to fail with an error naming it, rather than being skipped.

**A run's own record outlives its prompt.** Opening a past run shows the prompt text and configuration that run actually executed — frozen at the moment it started, so later edits to the prompt do not rewrite history, and deleting the prompt does not erase it. Its output and its files are still there too. Deleting the *run* is what removes all of that, and it removes it completely.
...
- A run whose task needs a suite service calls that service's tools directly, with no loading step, and completes the task end to end.
- A run started while a suite service is not answering fails with an error naming that service, instead of silently running without it.
...
- I delete a prompt, then list and open its runs and find them intact, each still showing the prompt text and configuration it ran with.
