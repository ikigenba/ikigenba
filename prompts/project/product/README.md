...
Every refusal or failure a sibling service receives from the completion and embedding surfaces carries one of these machine-readable codes beside its human-readable message. A code beginning `bad_` means the submission itself is wrong and no retry can clear it; `busy` and `unavailable` mean the condition belongs to the environment and a retry can clear it:

```
bad_request   bad_consumer   bad_origin   bad_name   bad_key   bad_context
bad_model     bad_config     not_found    busy       unavailable
```
...
- A completion the model cannot fulfil, or that fails at the provider, or that exceeds the runtime bound, or that has been abandoned by a lost execution once too often, is reported to the consumer as an explicit failure carrying its reason.
- A completion whose execution is lost while it runs — the service is killed mid-flight, or the worker carrying it dies — is carried to an answer anyway on a later attempt, without the consumer resubmitting it; no accepted completion is left in a state where neither side will ever act on it again.
- A refused submission and a failed completion each carry a stable code that says whether retrying could ever help, so a consumer can tell a passing unavailability apart from a permanently bad request.
- Restarting prompts repeatedly while a completion waits does not itself consume the item's tolerance for being abandoned: an ordinary restart returns the work to the queue rather than counting against it.
- The owner can see the queue's depth, the age of the oldest waiting item, and whether any work has been abandoned, from the running service, without opening its database.
