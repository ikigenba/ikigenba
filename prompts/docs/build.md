...
context. You read **only** `prompts/docs/brief.md` — never the plan, design, or
...
1. **Read the brief** — `prompts/docs/brief.md`. If it is missing or empty, there
...
5. **Commit this turn's increment** (never an empty commit) with a message naming
   ...
   Do **not** stage or commit `prompts/docs/brief.md` (it is gitignored). Then
   return `NEXT`.
...
- Never read `prompts/docs/plan/*`, `prompts/docs/design/*`, or
  `prompts/docs/product.md`. The brief is your only source.
- Never edit `prompts/docs/plan/STATUS.md` or flip a `⬜`/`✅` marker — that is
  verify's job alone.
- Never delete or edit `prompts/docs/brief.md`.
