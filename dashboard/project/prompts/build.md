...
isolated context. You read **only** `project/prompts/brief.md` — never the plan,
...
1. **Read the whole brief** — `project/prompts/brief.md`, **both** its contract
...
6. **Commit this turn's increment** (never an empty commit) with a message naming
   ...
   `project/prompts/brief.md` is gitignored, so `git add -A` will not stage it —
