...
3. **Confirm the tree is skip-free.** dashboard has **no live layer**, so no test
...
- Never delete a phase's `STATUS.md` line or `phase-NN.md` on anything short of a
  ...
  a skip is never acceptable green, and dashboard has no live layer, so there is
...
- `message` — one short, plain sentence describing what happened, e.g.
  `Phase 45 green: 2/2 ids covered, ratchet clean, deleted.` or
  `Phase 45 gap: R-O2IA-0JBL scan missing; recorded feedback attempt 2.` or
  `Phase 45 blocked: second stall, wrote blocked.md.`
