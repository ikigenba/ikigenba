# appkit/project — workspace layout

Everything the appkit service needs to be **designed, planned, and built**
...
root** (`appkit/`), which is also the directory the `ralph` build loop runs
from.

> **Status: scaffold.** The `project/` structure is established, but appkit
...
> lives in `appkit/docs/`, **not** here — `project/` is the development side.
