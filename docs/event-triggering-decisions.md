...
publishes named time events, `ralph` running sessions in reaction to them, and
...
process, **simplified ralph** (dropping the durable-intent model for in-memory
...
- **Originating gap.** Be able to create a scheduled research task in `ralph`
  (run periodically, synthesize, notify the owner), and be able to watch ralph
...
- **Engine-level dedup table stays deferred.** Consumers that need
  ...
  — or tolerate a rare duplicate effect — `ralph`, per the in-memory model in
  Section 3.)
...
## 3. `ralph`: event-trigger processing

- **ralph gains both roles** (it was neither producer nor consumer): it
  **consumes** cron's feed and **produces** outcome events.
- **A session declares an event trigger:** `event: cron.<name>`. This activates
  ralph's already-designed-but-deferred `trigger=event-subscribe` seam. The
  session↔event linkage lives in **ralph**, not in cron's payload. Multiple
  sessions may share one `cron.<name>`.
- **ralph consumes cron's feed via the fixed consumer engine** (Section 1).
...
- **Crash semantics (the accepted trade-off):** a crash loses the in-memory
  ...
  **that occurrence is missed** — the next cron tick recovers. No sweep, no
  ...
  skipped instead.) This is the best-effort philosophy applied to ralph; if
  paying twice for an LLM run ever proves painful, a single dedup constraint can
  be added later.

### Outcome events

- **ralph emits two terminal events** (source = `ralph`), each in the **same
  ...
  - *(Revised: `succeeded`, not `completed` — it matches ralph's existing
...
- **Outcome payload (minimal for now):**
  ...
  only on `run.failed`. The full report stays in ralph (read via MCP); the
  payload is widened in the future email phase.
- **ralph-as-producer wiring** mirrors crm/ledger: an `outbox.SchemaSQL`
  migration in ralph's DB, `Spec.Feed = "/feed"` + `Spec.Producer`, a **static**
  `Spec.Events` registry for the two compile-time-known types (not the live
  `Publishes` provider — that is cron-only), and an nginx `/feed` fragment. ralph
...
- **Minimal extension only.** `notify` adds `ralph` to `Consumes`
  (`["crm", "ralph"]`), subscribes to `run.succeeded` / `run.failed`, and fires
...
- **Two upstreams = two consumer loops.** notify runs one `consumer.Run` worker
  per upstream (crm, ralph), each with its own `feed_offset` cursor row — not one
...
3. ralph event-trigger processing: consume cron, in-memory fire-and-run with
...
for everything; cron must exist before ralph can consume it; ralph must emit
...
- **ralph durable-intent machinery** — the `run_intents` occurrence table, the
...
- **Live log viewer** — an operator-facing live follower (modeled on the prior
  ...
  (SSE rather than WebSocket; pretty-render ralph's stream-json run logs rather
