...
Five services (`crm`, `ledger`, `notify`, `dropbox`, `wiki`, `prompts`) carry a
...
appkit/
  ...
  server/          // Options/New/Run, routes, PRM, requireIdentityHeaders, securityHeaders, ungated /health (Envelope), Router hook
...
- **`appkit` = the deploy/serve chassis.** Everything that makes a process "an
  suite app": the six fixed verbs, config-from-env, the loopback HTTP server +
...
- **`agentkit` = the LLM/job engine.** `agent` (one-iteration agent loop), `job`
  ...
  manifests, or deploy. Used **only** by LLM services (`wiki`; future `prompts`).
...
| `Mount` | `string` | The `/srv/<app>/` prefix (manifest `MOUNT`). Composes `RESOURCE_ID`/`AUTH_SERVER` from `IKIGENBA_DOMAIN`; builds the PRM route. Dashboard = `/` + `Default:true`. |
...
| **crm / ledger / prompts** | Their `internal/<domain>` + `internal/mcp` tool descriptors + their `002_<svc>.sql` migration. crm/ledger are **producers** (`Feed:"/feed"`, `OUTBOX_RETENTION_*` extras, producer `restore` re-mints the epoch). prompts is currently a health-skeleton (universal keys only). |
...
- A **consumer** sets `Spec.Consumes` → appkit emits `CONSUMES=<names>` (purely
  ...
  be neither, like prompts; never both today).
