...
(crm → ledger → notify → wiki → prompts → dropbox) so the shared `tool()`/
...
1. **Add the prefix seam — defined once per service** (DECISIONS §1):
   ...
   `<svc>` ∈ {crm, ledger, notify, wiki, prompts, dropbox}.
...
5. **Wire version/service/reporter into the MCP Handler.** Extend the service's
   ...
   prompts), `Spec.Health` stays nil, so `rt.Health()` is nil and `details` renders
...
6. **In-description cross-references** (DECISIONS §1, third bullet): update every
   ...
   - prompts: `agent_whoami proves the auth chain` (describe.go) → rewrite to
     `ikigenba_prompts_health proves the auth chain` (also reflects whoami→health).
...
### Step 6 — prompts
Tools per `prompts/internal/mcp/tools.go` (`whoami→health` + the `prompts_session_*`
set). Update `prompts/internal/mcp/describe.go` prose (the `prompts_whoami` mention)
and `mcp_test.go`. `Spec.Health` nil. **Also** Step 8 covers the broader
`describe.go` prose pass — keep prompts' describe edit here limited to the tool-name
