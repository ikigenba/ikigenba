...
> architecture decision record for the suite's deploy story. It captures
...
- `bin/build` emitted **two** artifacts: `build/<app>.bin` (the Go binary) and
  ...
  composed the public `RESOURCE_ID`/`AUTH_SERVER` from `IKIGENBA_DOMAIN`+`MOUNT`,
...
### 1. The App Contract — "a suite app is exactly this"
...
- **No-arg invocation = serve.** All runtime config is read from the
  **environment** (`PORT`, `IKIGENBA_DOMAIN`, secrets injected by the launcher).
  The public `RESOURCE_ID` / `AUTH_SERVER` are **composed inside the binary**
  from `IKIGENBA_DOMAIN` + `MOUNT` — the logic the deleted wrapper used to do now
  lives in Go.
...
| `Mount` | `string` | The `/srv/<app>/` path prefix (manifest `MOUNT`). Used **in-binary** to compose `RESOURCE_ID`/`AUTH_SERVER` from `IKIGENBA_DOMAIN`, and to build the PRM route. For the dashboard this is the apex/`DEFAULT` case. |
...
/opt/<app>/
  ...
  bin/run -> ../current/<app>     # STABLE path ikigenba-launch execs (unchanged)
...
and
the regenerated file), so the baked `ikigenba-launch`, `bin/registry`, and the
...
(per-account bucket / box-local, per the ikigenba backup convention). Invokes
...
- write and **enable (not start)** the systemd unit with
  `ExecStart=/usr/local/bin/ikigenba-launch <app>` (the stable launcher
  contract);
...
3. **`ikigenba-launch`: additive first** — leave the baked launcher untouched
...
- **Stable paths must never break, including mid-swap.** `/opt/<app>/bin/run`
  ...
  file) are sourced/exec'd by `ikigenba-launch` and read by `bin/registry` + the
