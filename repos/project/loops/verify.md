...
2. **Run the full green suite** (from `repos/`), every command, and read the
   ...
   **`SKIP`ped** test) is a gap. **A skipped `R-XXXX-XXXX`-tagged test is a gap,
   ...
   entirely self-contained: **never run `bin/start` and never depend on a running
   suite.** The assembled-stack check is the suite's manual-layer item and is no
...
4. **Check coverage of every id in the brief's `### Ids to cover`.** Extract the
   ...
   - **actually runs under `cd repos && go test ./...`** — statically trace the
