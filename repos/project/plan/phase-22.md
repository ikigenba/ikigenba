...
- **Use.** `AppendOutcome`'s closure stops discarding its `context.Context` and
  passes it to the outbox `Append`. Every place the runner completes work on a
  fresh background context — the cancel path, the completion path, the reaper
  success call, and `Recover` — derives that context carrying the session's
  stored `CorrelationID` instead of a bare `context.Background()`. The
  detachment itself stays: a cancelled caller still must not kill a running
  session.
