...
`bin/bump notify <field>`) → `opsctl install` (which regenerates the on-box
manifest on every swap); provisioning is `opsctl setup notify`. The only `bin/*` scripts notify
