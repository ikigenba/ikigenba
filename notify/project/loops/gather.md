...
You are the **gather** step of the notify build loop, invoked in a fresh, isolated
...
2. **Read exactly that one phase body** — `project/plan/phase-NN.md`. It names
   ...
   **Done when:** list of `R-XXXX-XXXX` ids (or a **structural** phase with no ids
   — e.g. the nginx fragment's content-assertion ids are real ids, but the docs
   purge is structural with a content check).
...
- **Test placement — co-locate, never phase-name.** A phase is one package, so
  its tests live in that package: `notify/internal/web/web_test.go`, `package web`,
  ...
  `notify/etc/nginx.conf` from disk and asserts over its content; place it in the
  `web` package (e.g. `notify/internal/web/nginx_test.go`) or another focused
