...
You are the **verify** step of the notify build loop, invoked in a fresh, isolated
...
3. **Check coverage.**
   - **Code phase:** for **every** id listed under **Ids to cover**, confirm a
     ...
     `notify/etc/nginx.conf` and distinguishes the exact-match `= /srv/notify/`
     from the prefix `/srv/notify/`). **When uncertain that a test really asserts,
   ...
   - **Structural / docs phase** (Ids to cover = "(none — structural phase)"):
     ...
     `grep -i "no UI" notify/AGENTS.md` finds nothing, that `notify/AGENTS.md`
     states the landing-page truth, and that `notify/CLAUDE.md` still resolves to
     `notify/AGENTS.md` (the symlink is intact). The green suite plus the named
     check is the bar.
