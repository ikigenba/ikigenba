# Phase 23 — Register at lint tier `cheap`

*Realizes design Decision 22 (adopt the suite lint contract at tier `cheap`).*

A structural phase: commit `notify/.lint-tier` containing exactly `cheap`
...
**Done when:** `cat notify/.lint-tier` prints exactly `cheap`;
`bin/lint notify` (from the repo root) exits 0 reporting tier `cheap`; and
