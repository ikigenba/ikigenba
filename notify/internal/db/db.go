// Package db holds notify's embedded migration set and a thin migrate helper for
...
// §B / E2) — this package keeps only what is app-side: the *.sql files (embedded
// for Spec.Migrations) and the byte-equality guard that 002_feed_offset.sql
...
// Open opens notify's SQLite database with the chassis pragmas (delegates to
// appkit/db so there is one Open implementation across every service).
func Open(dbPath string) (*sql.DB, error) {
	return db.Open(dbPath)
}

// Migrate applies notify's embedded migrations against conn using appkit's
