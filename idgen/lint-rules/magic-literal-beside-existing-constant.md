...
```go
// Flagged: the 5-5 split appears as bare literals in three files while
// keyDigits, the constant that governs it, is referenced once.
const keyDigits = 10

return key[:5] + "-" + key[5:]
...
if len(part) != 5 { return false }
...
regexp.MustCompile(`^[0-9A-Z]{5}-[0-9A-Z]{5}$`)
```
...
const (
	keyDigits  = 10
	groupWidth = keyDigits / 2
)

return key[:groupWidth] + "-" + key[groupWidth:]
...
regexp.MustCompile(fmt.Sprintf(`^[0-9A-Z]{%d}-[0-9A-Z]{%d}$`, groupWidth, groupWidth))
```
