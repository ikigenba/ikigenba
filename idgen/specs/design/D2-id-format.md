...
- R-Y36K-9M90: Package `idgen` MUST fail loud at process start — panicking during package initialization — when the affine multiplier and 36⁸ are not coprime, and this guard MUST be verified by an ordinary deterministic test that drives the coprimality check with a non-coprime multiplier and observes the panic (a non-invertible map would make every existing id irrecoverable).
